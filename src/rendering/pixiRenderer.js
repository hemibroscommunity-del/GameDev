...
import { setSheen } from './lightfx/glint.js'; /* v2.3.2825: the permanent soft metal shine, behind ?sheen=1 */
...
export async function initPixiRenderer(canvas) {
  ...
  if (typeof window !== 'undefined') {
    window.__btLightFx = {
      ...
      glint: (p) => { lightFx.glint.force = (p == null ? null : (+p < 0 ? -1 : Math.min(1, +p))); },   /* v2.3.2825: -1 = hold every sweep off */
      /* v2.3.2825: the permanent sheen on/off, and a multiplier on its
