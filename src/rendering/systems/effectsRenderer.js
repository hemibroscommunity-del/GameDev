...
import { TILE, MINE_SPOT_R, FISH_CUE_DY, MINE_SEAT_DX, MINE_SEAT_DY, FISH_SEAT_DX, FISH_SEAT_DY } from '@/data/constants.js';   /* v2.3.2897: + the harvest seats */
...
/* ═══ v2.3.2897: WHERE THE LUMBERJACK AND THE COOK STAND -- ONE ANSWER ═══
 *
...
export class EffectsRenderer {
  ...
  /* ═══ v2.3.2897: THE VEIN OR POND A PEER IS WORKING ═══
  ...
  _updateGatherNodes(S, now) {
    this._peerWork = this._peerWorkNodes(S);   /* v2.3.2897: once a frame, for the vein here and the pond below */
    ...
    for (const node of nodes) {
      ...
      if (spriteTex) {
        ...
        /* v2.3.2897: ...or a PEER's vein.  The ore went over your own miner
  ...
  _updateFishingHole(S, now) {
    /* v2.3.2897: every hole drawn this frame, yours and your peers', for
    ...
    /* v2.3.2897: a PEER's line in the water.  Your own screen draws the
  ...
  /* The pond surface under a line: ripples and the bobber.  v2.3.2897: one
  ...
  /* v2.3.2897: the tree a peer is chopping, from their gather_node relay
  ...
  _updateRemoteExtraction(S, now) {
    ...
    for (const id in others) {
      ...
      /* ═══ v2.3.2897: AT THE TREE, AND AT THE FIRE ═══
      ...
      /* v2.3.1574 (owner: "the scale looks off for other players cooking and
         ...
         v2.3.2897: at the figure's own spot when it has one, as yours is. */
      ...
      /* ═══ v2.3.2303: WHICH SIDE OF THE TRUNK ═══
         ...
         v2.3.2897: the relayed tree decides it when there is one -- the
  ...
  _updateExtractionCue(S, now) {
    ...
    if (ex.skill === 'woodcutting' && this.chopSprite && this._chopFrames.length) {
      ...
      /* v2.3.2287: the vista curve, as on the fire figure above and on the
         ...
         v2.3.2897: the spot itself comes from chopStandInSpot, which the peer
    ...
    if (cookingCue && this.cookSprite && this._cookFrames.length) {
      ...
      /* v2.3.2287: the vista curve, as on the fire and chop figures.
         v2.3.2897: sampled, and the spot placed, by cookStandInSpot -- the
