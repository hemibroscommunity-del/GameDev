...
export class EffectsRenderer {
  constructor(layers) {
    ...
    const BOW_ART_VERSION = 962;   // 962: v2.3.2144 pinhole fill on the bow sheets (enclosed transparent specks, see playerSprites VERSION 100). 961: south bow "load" frame (f0) camera-left eye rebuilt from the matching right eye (was an oversized white-block sclera that read as keyed-out). 960: torso strips regenerated by the pants-confirmed waist detector (bow unchanged but 1px nw reslice). 959: torso strips re-cut at the skin->pants line (jog-legs seam). 958: bow recolored magenta->dark brown, cyan->black handle
    ...
    const JOG_LEGS_VERSION = 7;   // 7: v2.3.2144 pinhole fill -- the v2.3.1456 pass below left specks behind on every jog-legs sheet. 6: v2.3.1456 pinhole fill (enclosed transparent speckles inpainted); 5: drop the synthetic pants-fill rectangle (lift + real pants cover the seam)
