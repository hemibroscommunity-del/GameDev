...
import { placeSkillTraits, placeSkillTraitsFor, hideSkillTraits, placeStandInCape, selfCorpseUp, SWORD_SWING_MS, BOW_SHOT_MS, BOW_RELEASE_MS, standFootDy } from './entityRenderer.js'; /* v2.3.2190: the cape on an attack stand-in; v2.3.2281: is the corpse up; v2.3.2826: where a character's boots are */
...
import { CampfireFx } from '../campfireFx.js'; /* v2.3.2826: the lit-log campfire, in pixel art */
...
/* ═══ v2.3.2826: THE FIRE-LIGHTER STOOD 77 PX IN THE AIR ═══
...
export class EffectsRenderer {
  constructor(layers) {
    ...
    /* v2.3.2826: the campfire draws itself -- logs, flame, sparks and smoke as
   ...
   * code as your own.
   * v2.3.2826: that code is CampfireFx (rendering/campfireFx.js) now.  The
  ...
  _updateFiremaking(S, now) {
    ...
    sp.x = S.player.x;
    /* v2.3.2826: boots on your boots (see FIRE_FEET_ROW); was
    ...
    if (typeof window !== 'undefined') {
      ...
      window.__btFireFigure = () => ({
        ...
        /* v2.3.2826: where the figure's boots land (FIRE_FEET_ROW) */
  ...
  _updateRemoteExtraction(S, now) {
    ...
    for (const id in others) {
      ...
      sp.x = ox;
      /* v2.3.2826: the fire-lighter plants its boots on the peer's boots (see
  ...
  clear() {
    if (this._campfireFx) this._campfireFx.clear();   /* v2.3.2826 */
