...
/* ═══ v2.3.2855: THE LUMBERJACK'S BAKE, SHARED BY YOUR FIGURE AND A PEER'S ═══
 ...
 *  that window -- the fishing rod's lesson (v2.3.2854), where a pink tattoo
...
/* ═══ v2.3.2856: THE COOK'S DRAWINGS RIDE A LAYER OF THEIR OWN ═══
...
/* v2.3.2858: per figure -- the cook's and the fire-lighter's caches each hold
   this many (_peerStandInInk). */
const PEER_INK_CAP = 2;             /* drawn peers' layers held at once */
const PEER_INK_IDLE_MS = 15000;     /* released this long after the last frame that drew one */
const COOK_URL = { body: '/sprites/skills/cook-strip.webp', legless: '/sprites/skills/cook-strip-legless.webp' };
function _loadStandInImg(url) {
...
import { registerBowBodyFrames, BLOCK_STANDIN_HAND, BLOCK_OFFHAND, BLOCK_OFFHAND_PX, BLOCK_OFFHAND_ENABLED, BLOCK_OFFHAND_ART_ANG, BLOCK_ARM_FACING, blockArmTextureFrom } from '../blockArm.js'; /* v2.3.1785; v2.3.1833 the away-facing hand; v2.3.1864 the off-hand weapon; v2.3.2903 a peer's arm */
...
/* ═══ v2.3.2858: THE FIRE-LIGHTER'S DRAWINGS RIDE A LAYER OF THEIR OWN ═══
   The cook's arrangement (_bakeCookStrips), for the cook's reason: this bake is
   shared with every other player's fire-lighter on your screen (the SPEC
...
/* The drawn-peer layers, per figure (_peerStandInInk): the strip a variant
   ...
   strip; the fire-lighter has none. */
const STANDIN_INK = Object.freeze({
  cook: { fw: COOK_FW, fh: COOK_FH, url: (v) => (v ? COOK_URL.legless : COOK_URL.body), bake: _bakeCookSplit },
  fire: { fw: FIRE_FW, fh: FIRE_FH, url: () => FIRE_URL, bake: _bakeFireSplit },
});
...
/* ═══ v2.3.2760: DID A LOOPING ANIMATION PASS FRAME `target` SINCE LAST FRAME? ═══
...
export class EffectsRenderer {
  constructor(layers) {
    ...
    /* ═══ v2.3.2761: THE AXE IS COPPER ON A PINE HAFT, NOT THE MAGENTA KEY ═══
    ...
    /* v2.3.2856: your drawings, on a layer of their own over the cook (see
    ...
    this._cookFramesInk = null;            /* v2.3.2856: your drawings' layers -- null when you have none */
    ...
    /* v2.3.2858: your drawings' layer over the fire-lighter -- the cook's
       cookInkSprite, created straight after the body for the same reason:
       greaves, shirt and plate, created after it, cover it. */
    this.fireInkSprite = new Sprite();
    this.fireInkSprite.anchor.set(0.5, 1);
    this.fireInkSprite.visible = false;
    this.gestureLayer.addChild(this.fireInkSprite);
    this._fireFramesInk = null;
    ...
    const BOW_ART_VERSION = 964;   /* 964 (v2.3.2519): south bow RELEASE frame (f2) camera-left eye rebuilt from the matching right eye, on bow-south-body.png and bow-south-torso.png (tools/fix_bow_eye.mjs, now covering frames 0 and 2).  Owner: "south idle bow-shot, left eyeball all black" -- and it was: a solid dark rectangle with no sclera at all, beside a right eye with a proper white one.  Rendered at 12x in docs/triage-2026-09-14/lane-a/.  Version 961 below rebuilt the SAME eye on frame 0 for the opposite defect (an oversized white block) and its note called f2 "a dark-eyed release that is already fine"; it is not, and the tool's guard now accepts either shape.  Stand-ins get eyeT=null (the player's eye colour is never painted on one, TRAPS section 73), so these sheets ARE the eyes and only repainting them fixes this.  The .webp twins were deleted with the bump: the client prefers .webp and CI's optimize-assets mints them, so a stale twin would have kept serving the black eye. */   /* v2.3.2174: de-fringe sweep, see playerSprites VERSION 101. */   // 962: v2.3.2144 pinhole fill on the bow sheets (enclosed transparent specks, see playerSprites VERSION 100). 961: south bow "load" frame (f0) camera-left eye rebuilt from the matching right eye (was an oversized white-block sclera that read as keyed-out). 960: torso strips regenerated by the pants-confirmed waist detector (bow unchanged but 1px nw reslice). 959: torso strips re-cut at the skin->pants line (jog-legs seam). 958: bow recolored magenta->dark brown, cyan->black handle
  ...
  _bakeChopStrips() {
    ...
    /* v2.3.2855: the strips this replaces, released once nothing draws them.
  ...
  /* ═══ v2.3.2855: ANOTHER PLAYER'S LUMBERJACK CARRIES THEIR DRAWINGS ═══
   *
  ...
  /* ═══ v2.3.2856: THE COOK CARRIES YOUR DRAWINGS ═══
   *
  ...
  /* ═══ v2.3.2856: ANOTHER PLAYER'S COOK CARRIES THEIR DRAWINGS ═══
   *
   ...
   * PEER_INK_CAP at once, each released PEER_INK_IDLE_MS after nobody drew
   ...
   *
   * v2.3.2858: the same for another player's FIRE-LIGHTER, whose figure is
   * shared the same way: `kind` picks the figure (STANDIN_INK), each with a
   * cache of its own, so a campfire of cooks cannot starve the fire-lighters.
   * `variant` is the cook's legless strip; the fire-lighter has one strip. */
  _peerStandInInk(kind, o, variant, now) {
    const art = _peerChopArt(o);
    if (!art) return null;
    const spec = STANDIN_INK[kind];
    const key = (variant ? 'L' : 'B') + '|' + String(o.skin || '') + '|'
      + CHOP_INK_KEYS.map((k) => (artHasInk(art[k]) ? artHash(art[k]) : '')).join('.');
    const caches = this._peerInks || (this._peerInks = Object.create(null));
    const cache = caches[kind] || (caches[kind] = new Map());
    const hit = cache.get(key);
    if (hit) { hit.used = now; return hit.arr; }
    const pending = this._peerInkPending || (this._peerInkPending = Object.create(null));
    if (pending[kind]) return null;
    if (cache.size >= PEER_INK_CAP) {
      ...
      for (const [k2, e] of cache) if (e.used < lruUsed) { lruUsed = e.used; lruKey = k2; }
      if (lruKey == null || now - lruUsed < 1000) return null;
      this._dropPeerStandInInk(kind, lruKey);
    }
    pending[kind] = true;
    const skinT = skinTarget(o.skin) || DEFAULT_SKIN_TARGET;
  ...
  _dropPeerStandInInk(kind, key) {
    const cache = this._peerInks && this._peerInks[kind];
    const e = cache && cache.get(key);
    if (!e) return;
    cache.delete(key);
    const src = e.arr && e.arr[0] && e.arr[0].source;
    ...
    const slot = kind + 'Ink';   /* ent.cookInk / ent.fireInk */
    if (this._remoteSkillSprites) {
      for (const ent of this._remoteSkillSprites.values()) {
  ...
  _fetchAndBakeFire(inkOnly) {
    ...
    return _loadStandInImg(FIRE_URL).then((img) => {
      /* skinTarget() returns null for the 'default' pick, which means "the art
         ...
         point of the fix for anyone who never opened the skin picker. */
      const skinT = skinTarget(getSkin()) || DEFAULT_SKIN_TARGET;
      /* v2.3.2858: split, as the cook's is -- the figure every fire-lighter on
         your screen is drawn from, and your drawings' layer over it.  He faces
      ...
      /* v2.3.2858: the replaced textures, released as the cook's are (see
         ...
         players' fire-lighters draw from the figure, so they let go first. */
      const _sprites = [this.fireSprite, this.fireInkSprite];
      if (this._remoteSkillSprites) for (const ent of this._remoteSkillSprites.values()) if (ent.fire) _sprites.push(ent.fire);
      for (const arr of _old) {
        const src = arr && arr[0] && arr[0].source;
        if (!src) continue;
        for (const sp of _sprites) if (sp && sp.texture && sp.texture.source === src) sp.texture = Texture.EMPTY;
  ...
  update(S, viewW, viewH, now) {
    ...
    try { this._publishPeerBlockArms(S); } catch (e) { /* no arm this frame: the shield still draws */ }   /* v2.3.2903 */
  ...
  _updateProjectiles(S, now) {
    ...
    /* v2.3.1678: SNOWBALLS ARE NOT SLIME ORBS.
       ...
       v2.3.2217: THERE IS A SNOWBALL SPRITE NOW, and it is his own.
       `snowball.png` is cut from frame 5 of the south throw strip — the one
       frame that draws the ball in flight, standalone and free of the claw
       that wraps it in every held frame — so the ball in the air is the same
       drawing as the ball in his hand, not a lookalike.  Drawn at the strips'
       own 0.5 scale, which is what makes the two read as one object.
       (Owner, 2026-09-01: the orb below "looks like a plain white circle"
       next to the detailed one he holds.)

       The procedural orb is KEPT as the fallback, not deleted: the art is a
       per-zone asset, and a ball you cannot see is a ball you cannot dodge.
       It is a white orb with a soft rim, no filters — iOS WebGL, CLAUDE.md —
       so the softness is two stacked circles, the lantern-ring trick. */
    const snowballs = slimeProjs.filter((sp) => sp.kind === 'snowball');
    if (snowballs.length) {
      const ballTex = getSnowballTexture();
      for (const sp of snowballs) {
        if (ballTex) {
          let bs = sp._pixiSprite;
          if (!bs || bs.destroyed) {
            bs = new Sprite(ballTex);
            bs.anchor.set(0.5, 0.5);
            this.projectileLayer.addChild(bs);
            sp._pixiSprite = bs;
            this.slimeProjSprites.push({ proj: sp, sprite: bs });
          }
          if (bs.texture !== ballTex) bs.texture = ballTex;
          /* The strips render at 64/128; matching it puts the flying ball at
             exactly the size it was in his claw. */
          bs.scale.set(0.5);
          bs.rotation = 0;   /* the highlight is lit from one side — spinning it would read wrong */
          bs.x = sp.x;
          bs.y = sp.y;
          if (!bs.visible) bs.visible = true;
          continue;
        }
  ...
  /* ═══ v2.3.2760: SMOKE OFF THE PAN WHILE YOU FLIP ═══
  ...
  _updateFiremaking(S, now) {
    ...
    if (this.fireInkSprite) this.fireInkSprite.visible = false;   /* v2.3.2858 */
    ...
    if (fsp) {
      if (_fInk) {
        fsp.texture = _fInk;
        fsp.scale.set(sp.scale.x, sp.scale.y);
        fsp.x = sp.x; fsp.y = sp.y;
        fsp.visible = true;
      }
      sp._fireK = fi;   /* for mp-fireink: which frame both sprites are on */
    }
    /* ═══ v2.3.1713: THE SHIRT ═══
  ...
  _updateRemoteExtraction(S, now) {
    ...
    for (const ent of pool.values()) {
      /* v2.3.2858: was this peer's figure drawn last frame?  (The clock below.) */
      ent._wasOn = !!ent._onNow;
      ent._onNow = false;
      if (ent.chop) ent.chop.visible = false;
      if (ent.cook) ent.cook.visible = false;
      if (ent.cookInk) ent.cookInk.visible = false;   /* v2.3.2856 */
      if (ent.fire) ent.fire.visible = false;
      if (ent.fireInk) ent.fireInk.visible = false;   /* v2.3.2858 */
    ...
    for (const id in others) {
      ...
      /* v2.3.1749: anchor the animation to WHEN THIS PEER STARTED, not to the
         ...
         fire, saw it wrap. */
      /* v2.3.2858: ...and when they START AGAIN.  The code alone never changed
         between two fires lit in a row -- the pooled entry kept 'fire' while
         they walked -- so the second light was drawn frozen on the last frame
         for its whole length.  A figure that was not drawn last frame starts
         over (mp-fireink relights and watches for it). */
      if (ent._exCode !== code || !ent._wasOn) { ent._exCode = code; ent._exStart = now; }
      ent._onNow = true;
      ...
      /* v2.3.2855: a peer with drawings chops in a figure baked with them (and
      ...
      if (code === 'cook') {
        const _ci = this._peerStandInInk('cook', o, _leglessOn, now);
      ...
      /* v2.3.2858: and a drawn peer's fire-lighter, the same way -- their layer
         over the shared figure, on its frame, straight above it in the layer so
         their clothes cover it (_peerStandInInk). */
      if (code === 'fire') {
        const _fk = this._peerStandInInk('fire', o, false, now);
        const _fkt = _fk && _fk[_fiClamped];
        if (_fkt) {
          if (!ent.fireInk) {
            const ik = new Sprite();
            ik.anchor.set(0.5, 1);
          ...
          const ik = ent.fireInk;
          ik.texture = _fkt;
          ik.scale.set(sp.scale.x, sp.scale.y);
          ik.x = sp.x; ik.y = sp.y;
          ik.visible = true;
        }
        ent._fireInk = !!_fk;    /* for remoteSkillProbe */
        sp._fireK = _fiClamped;  /* for mp-fireink, as on the local figure */
      }
    ...
    for (const [id, ent] of pool) {
      if (!others[id]) {
        ...
        if (ent.cookInk) ent.cookInk.destroy();   /* v2.3.2856 */
        if (ent.fire) ent.fire.destroy();
        if (ent.fireInk) ent.fireInk.destroy();   /* v2.3.2858 */
    ...
    /* v2.3.2855: release the drawn peers' lumberjacks nobody has drawn for a
    ...
    /* v2.3.2856: the same for drawn peers' cooks -- v2.3.2858: and their
       fire-lighters (see _peerStandInInk). */
    if (this._peerInks) {
      for (const kind in this._peerInks) {
        const cache = this._peerInks[kind];
        if (!cache.size) continue;
        for (const [k, e] of cache) if (now - e.used > PEER_INK_IDLE_MS) this._dropPeerStandInInk(kind, k);
      }
    }
  }
  ...
  _updateRemoteSwordSwings(S, now) {
    ...
    for (const id in others) {
      ...
      if (!o || o._isDead) continue;
      /* ═══ v2.3.2903: A BASH IS NOT A SWING, HERE EITHER ═══
      ...
      /* ═══ v2.3.2905: SIZED AND PLANTED LIKE YOUR OWN SWING ═══
         ...
         v2.3.2905 note for why the flat REMOTE_SWING_SCALE made a peer shrink
      ...
      if (_jog) {
        ...
        this._placeJogLegs(set.jogLegs, set.jogLegsGear, {
          ...
          cutRow: swordTorsoCutRow(cfgKey, fi), jdir: _jdir, jfr: _jfr, mir: _rmir, s: _legS, x: sp.x, footY: _baseFootY,   /* v2.3.2905: _legS -- the local passes its own `s` */
  ...
  _ensureRemoteBowSet(id) {
    ...
    if (!set) {
      ...
      /* v2.3.1087: jogLegs/jogLegsGear created FIRST so they sit UNDER the body
         ...
         z-order. */
      /* v2.3.2903: + the block pose's three extra layers, on the structural
  ...
  _updateRemoteBowShots(S, now) {
    ...
    const others = (S && S.others) || {};
    const active = new Set();
    /* v2.3.2903: which facings can draw a peer's block pose, for
    ...
    for (const id in others) {
      ...
      /* ═══ v2.3.2903: ...AND THEIR BLOCK, DRAWN AS YOURS IS ═══
      ...
      if (!bodyFrames || !bodyFrames.length) continue;
      const n = bodyFrames.length;
      /* v2.3.2903: a block HOLDS one frame, as yours does (BLOCK_POSE_FRAME). */
      ...
      set.shieldLo.visible = false; set.offLo.visible = false; set.offHi.visible = false; set.shieldHi.visible = false;   /* v2.3.2903: parked, as yours are */
      const sp = set.body;
      const anchorY = cfg.feetY / cfg.fh;
      /* ═══ v2.3.1749: SCALE WITH THE BODY YOU ARE STANDING IN FOR ═══
         v2.3.1574 fixed exactly this for the harvest stand-ins ("the scale
         ...
         a peer attacking drew a full-size figure over a speck.  Same curve,
         same inputs, so the stand-in and the body shrink together. */
      /* ═══ v2.3.2905: SIZED AND PLANTED LIKE YOUR OWN BOW SHOT ═══
         ...
         entityRenderer's v2.3.2905 note on S._peerStandGeom has the mechanism).
      ...
      sp.x = (o.renderX != null) ? o.renderX : o.x;
      /* v2.3.2905: on the peer's measured feet, as yours sits on S._swordFootY. */
      ...
      place(set.weapon, _blk ? null : (weaponFrames && weaponFrames[fi]));   /* v2.3.2903: no bow in a block, as yours */
      const looks = {
        hair: o.hair, hairColor: o.hairColor,
        facialhair: o.facialhair, facialHairColor: o.facialHairColor,
        eyewear: o.eyewear,                                  /* v2.3.2361 */
    ...
    for (const [id, set] of this._remoteBowSprites) {
      ...
      set.body.visible = set.shirt.visible = set.chest.visible = set.legs.visible = set.weapon.visible = false;
      set.jogLegs.visible = set.jogLegsGear.visible = false;
      set.shieldLo.visible = set.offLo.visible = set.offHi.visible = set.shieldHi.visible = false;   /* v2.3.2903 */
      hideSkillTraits(set.traits);
      if (!others[id]) {
  ...
  /* ═══ v2.3.2903: A PEER'S SHIELD ARM, CUT FROM THEIR OWN BODY ═══
  ...
  /* v2.3.2903: a blocking peer's two hands -- the shield on the far side of
  ...
  /* ═══ v2.3.2903: THE OFF-HAND WEAPON, FOR ANY BLOCKING FIGURE ═══
  ...
  _updateExtractionCue(S, now) {
    ...
    if (this.cookInkSprite) this.cookInkSprite.visible = false;   /* v2.3.2856 */
    ...
    /* v2.3.2760: 800 -> 480ms -- the reel is now ~3s of quick cranking, and
    ...
    /* ═══ v2.3.2760: AT `ready` THE PAN IS STILL UNTIL YOU FLIP IT ═══
    ...
    if (ex.skill === 'woodcutting' && this.chopSprite && this._chopFrames.length) {
      ...
      const _gpC = gesturePose01(ex, now);   /* v2.3.2760: hand-paced; holds the raised axe until the first stroke */
      ...
      /* v2.3.2855: facing LEFT the figure is drawn flipped (below), so it takes
      ...
      sp._chopK = k;   /* v2.3.2855: the swing frame, for mp-chopink -- a cropped frame (v2.3.2775) no longer says which one it is */
      ...
      /* v2.3.2760: CROSSING the strike frame, not landing on it.  At the pace
      ...
      if (_crossedFrame(_chopL, k, CHOP_STRIKE_K)) {
        /* ═══ v2.3.2761: THE AXE SOUNDS LIKE AN AXE IN BARK ═══
    ...
    if (ex.skill === 'mining') {
      ...
      /* v2.3.2760: the frame the body is ACTUALLY showing.  This read the
      ...
      if (_crossedFrame(_mL, _mk, 4)) {
        /* v2.3.2760: the slam's sparks, moved here from ExtractionSwipeLayer's
