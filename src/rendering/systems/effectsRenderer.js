...
import { HitMaterialFx } from '../hitMaterialFx.js';              /* v2.3.2716: what a monster is made of, when it is hit */
...
/* v2.3.2200 -> v2.3.2716: the per-material hit debris (owner: "snow that
...
/* v2.3.2717: ONE sheet now -- the snowman's ice-burst plume is retired (see
...
/* v2.3.2504 -> v2.3.2716: the placeholder debris (owner §5.8: "the fallback
...
/* ═══ v2.3.2717: THE SNOWMAN'S ICE-BURST PLUME IS RETIRED ═══
   ...
   snowman's height.  Since v2.3.2716 a hit on a snowman throws crisp packed-snow
...
export class EffectsRenderer {
  constructor(layers) {
    ...
    /* ═══ v2.3.2740: THE STAFF CAST'S TWO SURFACES, BUILT AT CONSTRUCTION ═══
    ...
    /* v2.3.2716: the material hit reaction.  In front of a monster = the
  ...
  update(S, viewW, viewH, now) {
    ...
    /* v2.3.2740: the staff cast's sprite pools refill from zero each frame;
    ...
    /* v2.3.2740: after the projectiles, so a bolt fired this frame has
  ...
  _updateParticles(S, now) {
    ...
    if (S._impactRings) {
      for (let i = S._impactRings.length - 1; i >= 0; i--) {
        ...
        /* v2.3.2740: a staff bolt's crash ring is drawn by the staff cast
  ...
  _updateProjectiles(S, now) {
    ...
    for (const a of arrows) {
      ...
      if (isBowHeavy && ARROW_SPECIAL.frames.length) {
        ...
        if (MAGIC_BOLT_FRAMES.length) {
          this._placeMagicBolt(a, a._renderX, a._renderY, a.ang, fadeA, now, _liveBolts, _pk, S);   /* v2.3.2740: + S, for the caster's crystal */
        } else {
    ...
    for (const rp of remote) {
      ...
      if (rp.isStaff) {
        if (_remoteMagicSpec) {
          ...
          this._placeMagicBolt(rp, rp._renderX, rp._renderY, rp.ang, 0.95, now, _liveBolts, _pk, S);   /* v2.3.2740: + S */
        } else {
    ...
    /* v2.3.1334: reap magic-bolt sprites whose projectile is gone
       ...
       reaper below. */
    /* v2.3.2740: the breathing overlay (_boltGlow) is pooled in this same
  ...
  _placeMagicBolt(p, x, y, ang, alpha, now, liveSet, pk, S) {
    ...
    /* ═══ v2.3.2740: WHERE IT IS DRAWN IS ASKED OF THE STAFF CAST ═══
   ...
   * v2.3.2716: drawn by HitMaterialFx (rendering/hitMaterialFx.js) -- crisp
  ...
  /* v2.3.2716: the material hit-debris bursts (the note at the top of this run
  ...
  /* v2.3.2717: the snowman's per-hit plume is retired (tombstone near the old
  ...
  clear() {
    ...
    if (this._staffFx) this._staffFx.clear();   /* v2.3.2740: no sparks carried across a zone change */
    ...
    /* v2.3.2716: the material hit reaction's pieces (pooled; hidden, not destroyed) */
