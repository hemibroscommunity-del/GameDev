...
import { placeSkillTraits, placeSkillTraitsFor, hideSkillTraits, placeStandInCape, selfCorpseUp, SWORD_SWING_MS, BOW_SHOT_MS, BOW_RELEASE_MS, remoteBodyArt } from './entityRenderer.js'; /* v2.3.2190: the cape on an attack stand-in; v2.3.2281: is the corpse up */
...
import { recolorBodyToCanvas, recolorStandInSkin, DEFAULT_SKIN_TARGET, bodyArtSeg, skinTarget, pantsTarget, shoesTarget, getSkin, getPants, getShoes, onSkinChange, onPantsChange, onShoesChange, localBodyArt, artForFacing } from '../playerSkins.js'; /* v2.3.1710: + the skin-only stand-in recolour (the cook); v2.3.2429: + the player's own drawings */
...
/* Would the pre-flipped bake of this sheet differ from the plain one?  Asked
   ...
   the question.  (v2.3.2429 / v2.3.2431; module scope since v2.3.2821, when a
...
export class EffectsRenderer {
  constructor(layers) {
    ...
    /* Would the pre-flipped bake of this sheet differ from the plain one?  Asked
       ...
       a north sheet stamps the BACK canvases, so the front ones' symmetry is not
       the question. */
    /* (v2.3.2821: _twinWouldDiffer lives at module scope now -- a peer's
  ...
  /* ═══ v2.3.2821: A PEER'S SWING AND BOW SHOT WEAR THEIR DRAWINGS ═══
  ...
  _remoteBodyFramesFor(o, cfgKey, cfg, mirror) {
    ...
    const _pa = this._peerStandInArt(o, cfgKey, mirror);   /* v2.3.2821 */
    const key = cfg.bodyUrl + '|' + o.skin + '|' + o.pants + '|' + o.shoes + _pa.seg;   /* v2.3.2821: the SHEET, not the facing */
    ...
    try {
      const cv = recolorBodyToCanvas(img, skinTarget(o.skin) || DEFAULT_SKIN_TARGET, pantsTarget(o.pants), shoesTarget(o.shoes),
        null, cfg.fh, null, null, _pa.art, cfg.fw);   /* v2.3.2821: see _peerStandInArt */
  ...
  _remoteSheetFramesFor(o, url, fw, fh, dir, mirror) {
    ...
    const _pa = this._peerStandInArt(o, dir, mirror);   /* v2.3.2821: see _peerStandInArt */
    ...
    try {
      ...
      const cv = recolorBodyToCanvas(img, skinTarget(o.skin) || DEFAULT_SKIN_TARGET, pantsTarget(o.pants), shoesTarget(o.shoes),
        null, _fh, null, null, _pa.art, _fw);   /* v2.3.2821 */
  ...
  _updateRemoteSwordSwings(S, now) {
    ...
    for (const id in others) {
      ...
      const bodyFrames = this._remoteBodyFramesFor(o, cfgKey, cfg, mirror);   /* v2.3.2821: + the flip, for the drawings */
      ...
      const _torsoFrames = cfg.torsoUrl ? this._remoteSheetFramesFor(o, cfg.torsoUrl, cfg.fw, cfg.fh, cfgKey, mirror) : null;   /* v2.3.2821 */
      const _legArr = this._remoteSheetFramesFor(o, '/sprites/player/jog-' + _jdir + '-legs.png', null, null, _jdir, _rd.mirror);   /* v2.3.2355: square -- size read off the 128px sheet, not asserted as 256; v2.3.2821: + direction and flip, for the drawings */
      ...
      sp._qaFi = fi;   /* v2.3.2821: the frame index, for mp-peerattackink -- a cropped frame no longer says which it is */
