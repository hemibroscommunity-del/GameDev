...
export class EffectsRenderer {
  ...
  _updateRemoteSwordSwings(S, now) {
    ...
    for (const id in others) {
      ...
      /* ═══ v2.3.2921: SIZED AND PLANTED LIKE YOUR OWN SWING ═══
         ...
         v2.3.2921 note for why the flat REMOTE_SWING_SCALE made a peer shrink
      ...
      if (_jog) {
        ...
        this._placeJogLegs(set.jogLegs, set.jogLegsGear, {
          ...
          cutRow: swordTorsoCutRow(cfgKey, fi), jdir: _jdir, jfr: _jfr, mir: _rmir, s: _legS, x: sp.x, footY: _baseFootY,   /* v2.3.2921: _legS -- the local passes its own `s` */
  ...
  _updateRemoteBowShots(S, now) {
    ...
    for (const id in others) {
      ...
      /* ═══ v2.3.1749: SCALE WITH THE BODY YOU ARE STANDING IN FOR ═══
         ...
         a peer attacking drew a full-size figure over a speck.  Same curve,
         same inputs, so the stand-in and the body shrink together. */
      /* ═══ v2.3.2921: SIZED AND PLANTED LIKE YOUR OWN BOW SHOT ═══
         ...
         entityRenderer's v2.3.2921 note on S._peerStandGeom has the mechanism).
      ...
      sp.x = (o.renderX != null) ? o.renderX : o.x;
      /* v2.3.2921: on the peer's measured feet, as yours sits on S._swordFootY. */
