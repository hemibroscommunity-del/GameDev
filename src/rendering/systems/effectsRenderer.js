...
export class EffectsRenderer {
  ...
  _updateProjectiles(S, now) {
    /* v2.3.2901 QA probe: what each ordinary shot on screen was drawn WITH --
    ...
    for (const a of arrows) {
      ...
      if (_hotSpecial) {
        ...
        if (_shotLook) _shotLook.push({ who: 'self', kind: 'bigbolt', elem: a._fxElem || null, alpha: +fadeA.toFixed(3), life: a.life });   /* v2.3.2901 */
        ...
        if (MAGIC_BOLT_FRAMES.length) {
          ...
          if (_shotLook) _shotLook.push({ who: 'self', kind: 'bolt', elem: a._fxElem || null, alpha: +fadeA.toFixed(3), life: a.life });   /* v2.3.2901 */
        } else {
        ...
        if (_shotLook) _shotLook.push({ who: 'self', kind: 'arrow', color: elemColor, alpha: +fadeA.toFixed(3), life: a.life });   /* v2.3.2901 */
    ...
    for (const rp of remote) {
      ...
      /* ═══ v2.3.2901: DRAWN THE WAY THE SHOOTER DRAWS IT ═══
      ...
      if (!_remoteBasicBolt && !_remoteMagicSpec && !_remoteArrowSpec && !_remoteHot) this._updateProjectileTrail(rp, gfx, _rFade, !!rp.isStaff, _pk);   /* v2.3.2901: + the fade */
      if (rp.isStaff) {
        if (_remoteMagicSpec) {
          this._placeSpecialFx(MAGIC_SPECIAL, rp, rp._renderX, rp._renderY, rp.ang, _rFade, now, _liveBolts, _pk);   /* v2.3.2901: was a flat 0.95 */
        } else if (_remoteBasicBolt) {
          this._placeMagicBolt(rp, rp._renderX, rp._renderY, rp.ang, _rFade, now, _liveBolts, _pk, S);   /* v2.3.2841: + S; v2.3.2901: + the fade, was 0.95 */
          if (_shotLook) _shotLook.push({ who: rp.ownerId != null ? String(rp.ownerId) : '?', kind: rp.big ? 'bigbolt' : 'bolt', elem: rp._fxElem || null, alpha: +_rFade.toFixed(3), life: rp.life });   /* v2.3.2901 */
        } else {
        ...
        this._hotArrow.arrow(rp, rp._renderX, rp._renderY, rp.ang + bend, _rFade, _pk, now, false, 0);   /* v2.3.2901: + the fade */
      } else if (_remoteArrowSpec) {
        this._placeSpecialFx(ARROW_SPECIAL, rp, rp._renderX, rp._renderY, rp.ang + bend, _rFade, now, _liveBolts, _pk);   /* v2.3.2901: + the fade */
      } else {
        /* v2.3.2901: the element's colour and the fade, as yours (was tan at 0.9). */
        ...
        if (_shotLook && !rp.isSpecial) _shotLook.push({ who: rp.ownerId != null ? String(rp.ownerId) : '?', kind: 'arrow', color: _rElemColor, alpha: +_rFade.toFixed(3), life: rp.life });   /* v2.3.2901 */
