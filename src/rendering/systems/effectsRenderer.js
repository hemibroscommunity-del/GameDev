...
import { STAFF_BIG_BOLT_SCALE } from '@/data/gameSystems.js';     /* v2.3.2772: the one-bolt special's drawn size */
...
export class EffectsRenderer {
  constructor(layers) {
    ...
    /* ═══ v2.3.2771: THE STAFF CAST'S TWO SURFACES, BUILT AT CONSTRUCTION ═══
  ...
  update(S, viewW, viewH, now) {
    ...
    /* v2.3.2771: the staff cast's sprite pools refill from zero each frame;
    ...
    /* v2.3.2771: after the projectiles, so a bolt fired this frame has
  ...
  _updateParticles(S, now) {
    ...
    if (S._impactRings) {
      for (let i = S._impactRings.length - 1; i >= 0; i--) {
        ...
        /* v2.3.2771: a staff bolt's crash ring is drawn by the staff cast
  ...
  _updateProjectiles(S, now) {
    ...
    for (const a of arrows) {
      ...
      /* v2.3.1396: `ice: true` is set on EVERY staff special (it is the
         ...
         element. */
      /* v2.3.2772: ...except the one-bolt special (caps.bigOrb), which is
      ...
      if (isBowHeavy && ARROW_SPECIAL.frames.length) {
        ...
        /* v2.3.2772: the one-bolt special -- the basic bolt, bigger, leaving
        ...
        if (MAGIC_BOLT_FRAMES.length) {
          this._placeMagicBolt(a, a._renderX, a._renderY, a.ang, fadeA, now, _liveBolts, _pk, S);   /* v2.3.2771: + S, for the caster's crystal */
        } else {
    ...
    for (const rp of remote) {
      ...
      /* v2.3.2772: a peer's one-bolt special (rp.big) draws as their bolt. */
      ...
      if (rp.isStaff) {
        if (_remoteMagicSpec) {
          ...
          this._placeMagicBolt(rp, rp._renderX, rp._renderY, rp.ang, 0.95, now, _liveBolts, _pk, S);   /* v2.3.2771: + S */
        } else {
    ...
    /* v2.3.1334: reap magic-bolt sprites whose projectile is gone
       ...
       reaper below. */
    /* v2.3.2771: the breathing overlay (_boltGlow) is pooled in this same
  ...
  _placeMagicBolt(p, x, y, ang, alpha, now, liveSet, pk, S) {
    ...
    /* ═══ v2.3.2771: WHERE IT IS DRAWN IS ASKED OF THE STAFF CAST ═══
    ...
    /* v2.3.2772: the one-bolt special is this art drawn bigger -- the same
    ...
    if (fx) {
      ...
      /* v2.3.2772: softer on the big bolt -- at 1.7x the same additive copy
  ...
  clear() {
    ...
    if (this._staffFx) this._staffFx.clear();   /* v2.3.2771: no sparks carried across a zone change */
