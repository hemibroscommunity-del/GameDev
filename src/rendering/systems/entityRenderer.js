...
export class EntityRenderer {
  ...
  _updatePlayer(S, now) {
    ...
    if (S.rpg) {
      ...
      /* Swing trail.  For armed swings we draw the legacy fan-shaped
         arc sector centred on the hand pivot.  For unarmed (fist)
         swings we draw a vertical chop streak instead — a quadratic
         curve from overhead down to the strike point, matching the
         windup→punch motion in the new attack sprite. */
      if (swingActive) {
        ...
        if (wpn) {
          ...
          /* v2.3.247: unarmed chop streak.  Curves from above the
             hand (windup position) down to the strike point reach
             units forward of the player.  Special-attack chop is
             beefier (taller windup, longer reach, fatter stroke). */
