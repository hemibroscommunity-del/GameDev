...
function _placeGear(display, equip, pose, dir, frameIdx) {
  ...
  /* v2.3.1345: jog chain belt as its OWN gear layer.  Six rounds of baking
     the belt into the CHEST sheets each produced a new on-device artifact
     (sliding band, chain over the swinging arm, detached seam gaps), because
     the chest layer draws ABOVE the greaves and the arm.  The belt now ships
     as a body-frame-aligned sheet of its own
     (public/sprites/gear/belt/chainbelt/jog-<dir>.png, generated by
     tools/gen_jog_belt_table.py from measured per-frame waist anchors and
     clipped to the body silhouette) and renders on the _gearBelt sprite,
     added BELOW gearLegs at container setup — the owner rule "the belt
     always needs to be behind the chestplate and leg layers" holds by
     construction, and the band can never cover the arm or poke past the
     torso.  Shown only with the full set on the jog pose; other poses keep
     their baked belts (backlog). */
  const belt = display._gearBelt;
  if (belt) {
    let btex = null;
    if (sb && pose === 'jog'
        && equip && equip.chest && equip.chest !== 'none'
        && equip.legs && equip.legs !== 'none') {
      btex = getGearFrame('belt', 'chainbelt', pose, dir, frameIdx);
    }
    if (btex) {
      if (belt.texture !== btex) belt.texture = btex;
      belt.x = sb.x; belt.y = sb.y;
      belt.scale.x = sb.scale.x / DISPLAY_DS; belt.scale.y = sb.scale.y / DISPLAY_DS;
      if (!belt.visible) belt.visible = true;
    } else if (belt.visible) belt.visible = false;
  }
}
...
function createPlayerDisplay() {
  ...
  container._gearShirt = gearShirt; container._gearBelt = gearBelt; container._gearLegs = gearLegs; container._gearChest = gearChest; container._gearShoulders = gearShoulders; container._gearHead = gearHead;
...
export class EntityRenderer {
         ...
         weapon-arc to avoid a double image. */
      {
        ...
        if (_meleeSwing || _bowDraw) {
          ...
          for (const _k of ['_headwearSprite', '_facialHairSprite', '_hairSprite', '_shirtSprite',
            '_gearShirt', '_gearBelt', '_gearLegs', '_gearChest', '_gearShoulders', '_gearHead']) {
  ...
  _updatePlayer(S, now) {
    ...
    if (_swordSwing || _bowShot) {
      ...
      for (const _g of [display._gearShirt, display._gearBelt, display._gearLegs, display._gearChest,
