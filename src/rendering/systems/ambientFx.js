/* ═══ v2.3.2762: AMBIENT LIFE ON THE MAPS ═══
 *
...
const ZONE_FX = {
  /* v2.3.2762: k 0.55 -> 0.75 after an on/off diff of the real render showed
  ...
  /* ═══ v2.3.2762: WITH worldFx's AIR, NOT ON TOP OF IT ═══
...
export class AmbientFx {
  ...
  /* ═══ v2.3.2763: NOTHING SHINES AGAINST THE DASHBOARD ═══
  ...
  update(S, cx, cy, viewW, viewH, now, canvas) {
    ...
    const tsec = now / 1000;
    const M = 60;
    /* v2.3.2763: the dashboard's top edge, in world px, and the fade above it */
    ...
    if (typeof window !== 'undefined') {
      ...
      if ((this._probeAt & 15) === 0) {
        window.__btAmbient = { zone: this.zone, glows: this.glows.filter((g) => g.sp.visible).length, alive: Object.assign({}, alive),
          ...
          /* v2.3.2763: the brightest sprite within FADE of the dashboard (0 = the seam is clean) */
