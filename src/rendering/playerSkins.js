...
if (typeof window !== 'undefined') {
  ...
  /* v2.3.2852: the bytes one baked sheet holds (its packed canvas, before the
...
let _bakeTag = '';
/* ═══ v2.3.2431: THE FRAME WIDTH IS AN ARGUMENT, NOT A CONSTANT ═══
   Every per-frame step below used the module constant FRAME_W = 256, which was
   true of every sheet that had ever reached this function WITH ART: the stand
   and jog sheets are 256 wide per frame.  v2.3.2428 started sending the player's
   drawings to the sword, bow and jog-LEG stand-in strips as well, and those are
   122, 128, 130, 154, 160, 214, 320, 340 and 402 px per frame -- so `frames`
   came out as floor(w / 256), a number with no relationship to the sheet, and
   every mask, band and stamp below was measured in 256px windows straddling
   whichever figures happened to fall in them.

   MEASURED, before the fix, with a per-frame ink probe:
   ...
   A raised shield draws BLOCK_POSE_FRAME = 1 (effectsRenderer), so on three of
   the five bow facings the block pose showed NO tattoo at all -- the exact thing
   v2.3.2428 was written to fix -- and the trouser print flickered on every other
   jog-leg frame.
...
export function recolorBodyToCanvas(img, skinT, pantsT, shoesT, shirtT, targetH, eyeT, eyeRects, art, frameW, steadyArt, eyeBlankRects) {
  ...
  const shirtPx = shirtT ? _torsoBands(d, w, h, FW, frames) : null;
  ...
  const torsoPx = anyTat ? (shirtPx || _torsoBands(d, w, h, FW, frames)) : null;
  ...
  const pantsPaint = pantsPx
    ? litFabricMask(base, regionFromFeet(d, pantsPx, w, h, FW, PANTS_MAX_UP), w * h, PANTS_LIT_MIN) : null;
  /* Pattern first, print over it: a print is ON the fabric, and the fabric is
     what the pattern is. */
  if (pantsPat) stampPattern(d, w, h, FW, pantsPaint, pantsPat, !!art.mirror);
  ...
  if (wantPantsArt) stampRegion(d, w, h, FW, pantsPaint, art.pants, !!art.mirror, PANTS_BOX,
    _stampOpt(_rep && _rep.pants));
  if (shoesPat) {
    stampPattern(d, w, h, FW,
      litFabricMask(base, regionFromFeet(d, shoesPx, w, h, FW, SHOES_MAX_UP), w * h, SHOES_LIT_MIN),
      shoesPat, !!art.mirror);
  }
  ...
  /* v2.3.1949: face and arms.  `eachPiece` for the arms only -- a figure has
     ...
     nearer the camera. */
  if (skinPx) {
    const reg = splitSkinRegions(skinPx, torsoPx, w, h, FW);
    ...
    if (wantFaceTat) stampRegion(d, w, h, FW, reg.face, art.tattooFace, !!art.mirror, FACE_BOX,
      { underSkin: true, report: _rep && _rep.face, profile: _probe });
    if (wantArmTat) stampRegion(d, w, h, FW, reg.arms, art.tattooArm, !!art.mirror, ARM_BOX,
...
export function getBodyFrame(skinId, pantsId, shoesId, pose, dir, frameIdx, shirtT, shirtKey, eyeId, art, eyeStyleId) {
  const skinT = poseSkinTarget(skinTarget(skinId), pose), pantsT = pantsTarget(pantsId), shoesT = shoesTarget(shoesId);   /* v2.3.2852: poseSkinTarget */
...
function _pickupHeadCap() {
  ...
  for (const k of keys) {
    /* v2.3.2853: '|' OR '/es:' after the combo -- an eye style (v2.3.2643)
...
/* ═══ v2.3.2853: THE FACE TATTOO RIDES THE HEAD OVERLAYS ═══
...
function _buildPickupHeadSheet(key, pose, dir, skinT, pantsT, shoesT, eyeBlank, art = null, attempt = 0) {
  ...
  return loadImg(`/sprites/player/${pose}-${dir}-head.png?v=${SPRITE_VERSION}${_bust}`).then(img => {
    ...
    if (art && artHasInk(art.tattooFace)) _stampHeadInk(full, img, art);   /* v2.3.2853 */
    const cv = document.createElement('canvas');
...
export function getPickupHeadFrame(skinId, pantsId, shoesId, pose, dir, frameIdx, phase, eyeStyleId, art) {
  ...
  /* v2.3.2853: the drawings, resolved for this facing exactly as the body
...
export function preloadBodyAll() {
  ...
  /* v2.3.2852: the default combo used to return here with nothing to bake.  It
  ...
  const bake = (pose, dir, a) => {
    const sT = poseSkinTarget(skinT, pose);   /* v2.3.2852 */
    if (plain && !sT) return;             /* v2.3.2852: the default combo bakes only those three */
  ...
  /* v2.3.2853: through _headSheetKey, so it is the key getPickupHeadFrame will
  ...
  if (!plain && _pickupHeadSheets[headKey] === undefined) tasks.push(_buildPickupHeadSheet(headKey, 'pickup', 'south', skinT, pantsT, shoesT, _hBlank && _hBlank.rects, _hArt));   /* v2.3.2852: !plain -- the default combo never reached here before */
...
export function preloadJogHeadOverlays() {
  ...
  /* v2.3.2853: with the local drawings, and the mirrored bake as well for the
  ...
  for (const [pose, dir] of want) {
    ...
    for (const a0 of arts) {
      ...
      const key = _headSheetKey(skinId, pantsId, shoesId, pose, dir, blank, a);
      if (_pickupHeadSheets[key] === undefined) tasks.push(_buildPickupHeadSheet(key, pose, dir, poseSkinTarget(skinT, pose), pantsT, shoesT, blank && blank.rects, a));   /* v2.3.2852: poseSkinTarget */
    }
  }
  return Promise.all(tasks);
}
  ...
  for (const pose of ['stand', 'jog', 'hit']) {   /* v2.3.1477: hit ships gear now */
    for (const dir of SOURCE_DIRS) {
      ...
      for (const a of (artM && MIRRORED_SOURCE_DIRS.indexOf(dir) !== -1) ? [art, artM] : [art]) {
        ...
        if (_bodySheets[key] === undefined) tasks.push(buildBodySheet(key, pose, dir, poseSkinTarget(skinT, pose), pantsT, shoesT, shirtT, eye && eye.t, a, blank && blank.rects));   /* v2.3.2852: poseSkinTarget */
      }
    }
  }
...
function _dropArtSheets() {
  ...
  /* v2.3.2853: and the head overlays that carried a face drawing.  Destroyed
...
/* v2.3.2853: the local player's head overlays again, after a drawing edit
...
function _onArtChanged() {
  ...
  _artPrewarmT = setTimeout(() => {
    ...
    /* v2.3.2853: the head overlays too, so the next pickup shows the new
