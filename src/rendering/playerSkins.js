...
if (typeof window !== 'undefined') {
  ...
  /* v2.3.2860: the bytes one baked sheet holds (its packed canvas, before the
...
export function getBodyFrame(skinId, pantsId, shoesId, pose, dir, frameIdx, shirtT, shirtKey, eyeId, art, eyeStyleId) {
  const skinT = poseSkinTarget(skinTarget(skinId), pose), pantsT = pantsTarget(pantsId), shoesT = shoesTarget(shoesId);   /* v2.3.2860: poseSkinTarget */
...
function _pickupHeadCap() {
  ...
  for (const k of keys) {
    /* v2.3.2861: '|' OR '/es:' after the combo -- an eye style (v2.3.2643)
...
/* ═══ v2.3.2861: THE FACE TATTOO RIDES THE HEAD OVERLAYS ═══
...
function _buildPickupHeadSheet(key, pose, dir, skinT, pantsT, shoesT, eyeBlank, art = null, attempt = 0) {
  ...
  return loadImg(`/sprites/player/${pose}-${dir}-head.png?v=${SPRITE_VERSION}${_bust}`).then(img => {
    ...
    if (art && artHasInk(art.tattooFace)) _stampHeadInk(full, img, art);   /* v2.3.2861 */
    const cv = document.createElement('canvas');
...
export function getPickupHeadFrame(skinId, pantsId, shoesId, pose, dir, frameIdx, phase, eyeStyleId, art) {
  ...
  /* v2.3.2861: the drawings, resolved for this facing exactly as the body
...
export function preloadBodyAll() {
  ...
  /* v2.3.2860: the default combo used to return here with nothing to bake.  It
  ...
  const bake = (pose, dir, a) => {
    const sT = poseSkinTarget(skinT, pose);   /* v2.3.2860 */
    if (plain && !sT) return;             /* v2.3.2860: the default combo bakes only those three */
  ...
  /* v2.3.2861: through _headSheetKey, so it is the key getPickupHeadFrame will
  ...
  if (!plain && _pickupHeadSheets[headKey] === undefined) tasks.push(_buildPickupHeadSheet(headKey, 'pickup', 'south', skinT, pantsT, shoesT, _hBlank && _hBlank.rects, _hArt));   /* v2.3.2860: !plain -- the default combo never reached here before */
...
export function preloadJogHeadOverlays() {
  ...
  /* v2.3.2861: with the local drawings, and the mirrored bake as well for the
  ...
  for (const [pose, dir] of want) {
    ...
    for (const a0 of arts) {
      ...
      const key = _headSheetKey(skinId, pantsId, shoesId, pose, dir, blank, a);
      if (_pickupHeadSheets[key] === undefined) tasks.push(_buildPickupHeadSheet(key, pose, dir, poseSkinTarget(skinT, pose), pantsT, shoesT, blank && blank.rects, a));   /* v2.3.2860: poseSkinTarget */
    }
  }
  return Promise.all(tasks);
}
  ...
  for (const pose of ['stand', 'jog', 'hit']) {   /* v2.3.1477: hit ships gear now */
    for (const dir of SOURCE_DIRS) {
      ...
      for (const a of (artM && MIRRORED_SOURCE_DIRS.indexOf(dir) !== -1) ? [art, artM] : [art]) {
        ...
        if (_bodySheets[key] === undefined) tasks.push(buildBodySheet(key, pose, dir, poseSkinTarget(skinT, pose), pantsT, shoesT, shirtT, eye && eye.t, a, blank && blank.rects));   /* v2.3.2860: poseSkinTarget */
      }
    }
  }
...
function _dropArtSheets() {
  ...
  /* v2.3.2861: and the head overlays that carried a face drawing.  Destroyed
...
/* v2.3.2861: the local player's head overlays again, after a drawing edit
...
function _onArtChanged() {
  ...
  _artPrewarmT = setTimeout(() => {
    ...
    /* v2.3.2861: the head overlays too, so the next pickup shows the new
