...
export function stampRegion(d, w, h, frameW, mask, art, mirror, box, opts) {
  ...
  /* ═══ v2.3.2783: A BOX THE CALLER ALREADY KNOWS ═══
  ...
  for (let f = 0; f < frames; f++) {
    ...
    /* v2.3.2783: with a fixed box the piece is only asked "is there any region
    ...
    if (fixedBoxes) {   /* v2.3.2783: see `fixedBoxes` above */
 ...
 */
/* ═══ v2.3.2516: THE HEAD'S OWN BOX, WALKED DOWN FROM THE CROWN ═══
...
export function splitSkinRegions(skin, torso, w, h, frameW) {
  ...
  for (let f = 0; f < frames; f++) {
    ...
    const head = _headBoxInFrame(skin, w, h, x0, x1);   /* v2.3.2516 */
    for (let y = 0; y < h; y++) {
      const row = y * w;
      for (let x = x0; x < x1; x++) {
        if (!skin[row + x]) continue;
        if (y < top) face[row + x] = 1;
        /* v2.3.2516: the rest of the head, when the band's first row cut it in
...
/* ═══ v2.3.2783: FACE / TORSO / ARMS FROM SEEDS PLACED BY HAND ═══
 *
