/* ═══ v2.3.2841: THE STAFF CAST — CHARGE, RELEASE, A LIVING BOLT, A HOT HIT ═══
 *
...
/* v2.3.2842: the one-bolt special (gameSystems STAFF_BIG_BOLT_*) is the same
...
export function staffCastPose(now, castAt, castAng, charge, rhythm, mirror, pixelSteps, big) {
  ...
  /* v2.3.2842: the one-bolt special kicks harder and settles slower. */
...
export class StaffCastFx {
  ...
  _onCrash(x, y, ramp, pk, now, big) {
    ...
    /* v2.3.2842: the one-bolt special lands like three orbs at once -- more
  ...
  bolt(p, x, y, ang, pk, now, S) {
    ...
    if (p._fxSeen == null) {
      ...
      /* v2.3.2893: a peer's bolt carries its caster's element now (relayed on
    ...
    p._fxRx = x; p._fxRy = y;
    /* the element halo, under the art (v2.3.2842: wider and brighter round
  ...
  update(S, now, selfCorpse) {
    ...
    for (let i = this.flashes.length - 1; i >= 0; i--) {
      ...
      if (fl.kind === 2) {
        /* v2.3.2842: the one-bolt special's release -- a bigger flash that
      ...
      if (fl.kind === 3) {
        /* v2.3.2842: the one-bolt special's crash -- a bigger white-out and a
