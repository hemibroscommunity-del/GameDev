...
/* v2.3.2802: the one-bolt special (gameSystems STAFF_BIG_BOLT_*) is the same
...
export function staffCastPose(now, castAt, castAng, charge, rhythm, mirror, pixelSteps, big) {
  ...
  /* v2.3.2802: the one-bolt special kicks harder and settles slower. */
...
export class StaffCastFx {
  ...
  _onCrash(x, y, ramp, pk, now, big) {
    ...
    /* v2.3.2802: the one-bolt special lands like three orbs at once -- more
  ...
  bolt(p, x, y, ang, pk, now, S) {
    ...
    /* Trail: one spark per ~6 px of flight, laid along the segment, so the
       spacing is the same at 30 fps as at 60.  v2.3.2802: the big bolt lays a
    ...
    p._fxRx = x; p._fxRy = y;
    /* the element halo, under the art (v2.3.2802: wider and brighter round
  ...
  update(S, now, selfCorpse) {
    ...
    for (let i = this.flashes.length - 1; i >= 0; i--) {
      ...
      if (fl.kind === 2) {
        /* v2.3.2802: the one-bolt special's release -- a bigger flash that
      ...
      if (fl.kind === 3) {
        /* v2.3.2802: the one-bolt special's crash -- a bigger white-out and a
