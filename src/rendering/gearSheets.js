...
/* v2.3.2748: every frame of the sheet a gear texture belongs to.  A cropped
 ...
 *  to (dw x dh) at (dx, dy).  Handles cropped frames (v2.3.2748) and plain
...
function buildSheet(key, slot, item, pose, dir, attempt = 0, stampArt = null) {
  ...
  return loadImg(`/sprites/gear/${slot}/${item}/${pose}-${dir}.png?v=${GEAR_VERSION}${bust}`).then(rawImg => {
    ...
    /* v2.3.2748: crop the empty space off every frame before upload -- see
    ...
    /* v2.3.2748: a cropped sheet's resource is our packed canvas, which has no
