 ...
 *
 * ═══ v2.3.2781: A SOFT SHINE THAT STAYS (PREVIEW, OFF UNLESS ASKED FOR) ═══
 *
...
void main()
{
    ...
    /* v2.3.2781: the sheen.  How bright the ART was here, before the metal's
  ...
  return Filter.from({
    ...
    resources: {
      glintUniforms: {
        ...
        uSheen: { value: 0, type: 'f32' },                                   /* v2.3.2781 */
...
/* v2.3.2781: `sheen` is the permanent shine's strength at its brightest
...
/* ═══ v2.3.2781: THE SHEEN'S SWITCH -- OFF UNLESS THIS DEVICE ASKED ═══ */
...
export class GlintSystem {
  constructor() {
    ...
    this._pool = [];
    this.force = null;           /* QA/pictures: a fixed sweep progress, 0-1; -1 = no sweep anywhere (v2.3.2781) */
  ...
  _targets(S, er, fx, zone) {
    ...
    if (pd && !pd.destroyed && rpg) {
      ...
      if (cm) {
        ...
        /* v2.3.2781: jogging in a full set, the armour IS the body sprite */
    ...
    if (er && er.otherPlayerDisplays && others) {
      for (const [id, d] of er.otherPlayerDisplays) {
        ...
        if (cm) {
          ...
          if (d.visible && d._fullsetOn) pushVisible(s, d._spriteBody);   /* v2.3.2781 */
  ...
  /* `sheen` (v2.3.2781): null when the permanent shine is off, else
  ...
  /* QA probe (v2.3.2781): which slots are lit right now, and whether that
