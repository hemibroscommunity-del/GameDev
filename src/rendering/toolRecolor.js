/* ═══ v2.3.2761: THE MAGENTA TOOLS GET THEIR MATERIALS ═══
 *
 ...
 * code-drawn fire in another session -- owner, v2.3.2761 -- so it is left
 ...
 * `onlyMask` (v2.3.2822), when given, limits it to pixels that are key in THAT
...
export function recolorToolKeyCanvas(cv, spec, scale = 1, onlyMask = null) {
  ...
  const n = recolorToolKey(img.data, cv.width, cv.height, spec, scale, onlyMask);   /* v2.3.2822: + onlyMask */
