...
import { propSpriteSources, propSpriteSourcesIn, propAnimStrips, zoneDecorSources } from '../data/worldProps.js'; /* v2.3.1775: scenery shares this registry; v2.3.2061: + animated strips; v2.3.2651: + per-zone decor; v2.3.2850: + per-hub split */
import { loadCroppedStrip } from './gearSheets.js'; /* v2.3.2850: the NPC walk strips load cropped */
...
/* v2.3.2850: the hub whose NPCs and props load and free with it (loadTownScenery) */
const TOWN = 'town';
let _done = null;
...
export function npcSpriteSources() {
  /* v2.3.2850: the world figures, the walk strips and town's props are NOT in
  ...
  for (const n of NPC_DATA || []) {
    ...
    /* v2.3.2045: a WALKING NPC's per-direction strips. Listed from NPC_DATA
       ...
       forbids.  (v2.3.2850: listed from the same table by townScenery()
...
export function loadNpcSprites() {
  ...
  /* v2.3.2850: the walk strips and the animated props used to be sliced here,
     after the same promise.  They are town's, so they moved to
     loadTownScenery with the rest of town. */
  return _done;
}

/* ═══ v2.3.2850: TOWN'S NPCs AND BUILDINGS LOAD AND FREE WITH TOWN ═══
 *
