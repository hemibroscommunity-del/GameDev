...
import { getCape } from './traits/capeCatalog.js';        /* v2.3.2509: the worn cape, in the preview */
...
/* ═══ v2.3.2509: THE CAPE'S OWN CACHE-BUST ═══
...
export async function drawCharacterPortrait(canvas, opts) {
  ...
  /* ═══ v2.3.2509: THE CAPE, IN THE EQUIPMENT PREVIEW ═══
  ...
  /* v2.3.2509: the cape's PANELS, after the slung shield and before the body --
  ...
  if (hairImg && hairMeta) {
    ...
    if (hwImg && hwMeta && hwMeta.clipsHair && maskImg) {
      ...
      /* v2.3.2509: the HOOD clips the hair, the same rule and the same
  ...
  /* v2.3.2509: the cape's FRONT half, in the world's own order -- above the
...
export function prewarmPortraitDirs(opts) {
  ...
  /* v2.3.2509: the cape too.  ANIMATION PRELOADING IS LAW (CLAUDE.md): a cape
