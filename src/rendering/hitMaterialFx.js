/* ═══ v2.3.2784: A MONSTER REACTS LIKE WHAT IT IS MADE OF ═══
 *
...
/* Dust / powder / smoke / steam: dithered discs, generated (radius 1..7).
   ...
   as nothing when the disc was flat -- measured on the v2.3.2784 captures). */
...
export class HitMaterialFx {
  ...
  _burst(b, S, now) {
    ...
    /* v2.3.2784: a PROP's burst (combatHelpers.spawnPropDebris, v2.3.2730) is
    ...
    /* ═══ v2.3.2785: A BOLT BLASTS ALL ROUND -- IN EVERY BURST ═══
       ...
       in 20 on slime and rock, measured on the merged v2.3.2784 build).  A
  ...
  report(now) {
    ...
    for (const b of this._bursts) {
      ...
      out.push({
        /* v2.3.2785: + tint, the colour the burst was handed (a blue slime's
