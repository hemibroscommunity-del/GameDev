...
import { freeZoneMap, isZoneMapResident, preloadStartZoneMap } from '@/rendering/tiledMaps.js'; /* v2.3.1405: map eviction + sync residency check; v2.3.2850: + town's own map */
import { loadTownScenery, freeTownScenery, townSceneryReady, townSceneryLoading } from '@/rendering/npcSprites.js'; /* v2.3.2850: town's NPCs + buildings load and free with town */
...
/* ═══ v2.3.2850: TOWN'S NPCs AND BUILDINGS, HELD AND FREED IN ONE PLACE ═══
 *
...
export function handleZoneTransitions(S, ptx, pty, _zone, W, H) {
  ...
  syncTownScenery(S);   /* v2.3.2850: town's NPCs + buildings, whichever way you arrived */
        ...
        if (_hubExits) {
          ...
          if (bestExit) {
            ...
            /* Zone exits — open to all players (quest gate removed) */
            {
              ...
              } else if (!isZoneMapResident(_tz) || (_tz === 'town' && !townSceneryReady())) {   /* v2.3.2850: town's NPCs + buildings leave with town (its map too, which the first test already covers) */
