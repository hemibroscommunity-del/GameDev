...
var PROJ_BODY = {
  ...
  /* v2.3.2807: the special is now DRAWN as the pine arrow at 62.8 px (hotArrowFx
...
/* v2.3.2511: and it may not reach more than half the special arrow's own drawn
   ...
   v2.3.2807: the white-hot special (hotArrowFx) is drawn at the same 62.8. */
...
/* ═══ v2.3.2804: WHERE A SHOT LANDS IN THE BODY IT HIT ═══
   ...
   thin).
   v2.3.2805: ...round the TORSO (gameSystems monsterTorsoY), which is where a
...
var LAND_CORE = { fodder: [10, 7], fireGoblin: [8, 12], snowman: [10, 9], mummy: [8, 11], skeleton: [8, 11],
  fishman: [8, 12], bogLurker: [8, 12], rockmonster: [12, 14], thornShambler: [12, 14] };   /* v2.3.2805: + the tall figures' torsos */
...
function _landCore(m, S) {
  ...
  if (core) {
    var k = monsterDrawScale(m, S && S.currentZone);   /* v2.3.2805 */
...
function _pickLanding(S, a, m, tipX, tipY) {
  ...
  var ly = my - torsoLift(m, S && S.currentZone) + _tri() * core[1];   /* v2.3.2805: round the torso */
...
/* ═══ v2.3.2805: THE CIRCLE A SHOT MUST TOUCH SITS WHERE IT WAS AIMED ═══
...
/* A hit, on screen, at the point (tx, ty) where the shot landed: the monster's
   recoil and flash (v2.3.2200), its material (v2.3.2803), the sound of what was
   hit (v2.3.2511), a bolt's crash (v2.3.2505 / v2.3.2801) and a plain arrow's
...
function _projImpactFx(S, a, m, fx, tx, ty) {
  ...
  m._hitFlash = now;
  /* v2.3.2801: the staff cast draws a bolt eased off the crystal over its first
  ...
  /* v2.3.2803: AN ARROW PUNCHES, A BOLT BLASTS -- the material leaves from
  ...
  if (fx.staff) {
    /* v2.3.2505: the crash where the orb is.  v2.3.2730: through combatHelpers'
       orbCrashFx, the one a bolt stopped by a prop uses too; v2.3.2801: which
...
function _quarryInBox(S, a, b) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    var vx = fx - px, vy = (fy - _projCentreLift(S, a, m)) - py;   /* v2.3.2805: the hit test's own centre */
...
export function firstSightHit(S, ox, oy, ang, maxLen, opts) {
  ...
  if (list) {
    for (var i = 0; i < list.length; i++) {
      ...
      /* v2.3.2805: the monster the shot is aimed at, round its torso -- as the hit test will */
  ...
  var setRpgState = deps.setRpgState,
    setLevelUpMsg = deps.setLevelUpMsg;
        ...
        if (S.arrows && S.arrows.length > 0) {
          ...
          /* v2.3.2805: ...at the TORSO (lockShotPoint), and the target's hit circle is centred there for this shot (_projCentreLift) */
          var lockPt = lockShotPoint(S.lockedTarget && S.lockedTarget.ref, S.currentZone);
          var lockId = (lockPt && S.lockedTarget.ref.id != null) ? S.lockedTarget.ref.id : null;   /* v2.3.2805: see _projCentreLift */
          ...
          S.arrows = S.arrows.filter(function (a) {
            ...
            if (a.stuckIn) {
              ...
              var _smy = ((typeof _sm.renderY === 'number') ? _sm.renderY : _sm.y) - torsoLift(_sm, S.currentZone);   /* v2.3.2805: the torso */
              var _stX = _smx + (a._stickOx || 0), _stY = _smy + (a._stickOy || 0);
              if (a._landFx) {
                /* ═══ v2.3.2804: THE SPECIAL FLIES IN, THEN LANDS ═══
                   ...
                   arrow's did (v2.3.2803).  Its chip ticks were already
            ...
            /* ═══ v2.3.2804: LANDING -- the shot has hit, and flies on in ═══
            ...
            if (S.monsters) S.monsters.forEach(function (m) {
              ...
              var _mProjY = _hitBaseY - _projCentreLift(S, a, m);   /* v2.3.2805: round the torso, for the monster it was aimed at */
              ...
              if (_segD < _hitRE) {
                ...
                /* Hit-reaction (ranged variant) — mirrors the melee path.
                   ═══ v2.3.2804: THE PICTURE WAITS FOR THE SHOT TO LAND ═══
                   The recoil, the white flash, the material burst (v2.3.2803)
                ...
                /* ═══ v2.3.2511: AN ARROW SOUNDS LIKE WHAT IT HIT ═══
                   ...
                   v2.3.2804: played by _projImpactFx now, when the shot LANDS in
                   the body, so the sound and the burst are one event. */
                var _hitMat = hitMaterialOf(m.archetype || m.type);
                /* v2.3.2804: the "blood spray on bow hits" is retired -- seven flat
                   ...
                   generic sprays (v2.3.2803 retired the splinters, v2.3.2801 the
                ...
                /* Magic orb crash & dissipate — element-tinted impact ring
                   ...
                   v2.3.2804: drawn by _projImpactFx where the orb LANDS; the
                ...
                if (a.isStaff) {
                   ...
                   * one that did not. */
                  /* ═══ v2.3.2804: ...AND NOW THE ORB IS IN THE BODY WHEN IT DOES ═══
                     The two rings and the staff cast's burst (v2.3.2801) are
                ...
                /* v2.3.2801: a staff bolt's hit is drawn once, by its crash above --
                   ...
                   v2.3.2803: and the arrow's generic splinter-and-dust spray
                ...
                /* Staff projectiles are magic — no physical shaft to
                   ...
                   crash above (v2.3.2801), and the material reaction (v2.3.2803). */
                ...
                /* ═══ v2.3.2804: THE SHOT FLIES ON INTO THE BODY, AND LANDS THERE ═══
                ...
                /* ═══ v2.3.2731: ...OR IT SNAPS ═══
                   ...
                   broken one could not.  v2.3.2804: rolled here, on the hit,
            ...
            /* v2.3.2804: ...and a shot flying on into the body it hit survives
            ...
            if (a._land) return true;   /* v2.3.2804: no prop stop for a shot that has already hit */
