...
/* ═══ v2.3.2785: WHERE A SHOT LANDS IN THE BODY IT HIT ═══
   ...
   thin).
   v2.3.2786: ...round the TORSO (gameSystems monsterTorsoY), which is where a
...
var LAND_CORE = { fodder: [10, 7], fireGoblin: [8, 12], snowman: [10, 9], mummy: [8, 11], skeleton: [8, 11],
  fishman: [8, 12], bogLurker: [8, 12], rockmonster: [12, 14], thornShambler: [12, 14] };   /* v2.3.2786: + the tall figures' torsos */
...
function _landCore(m, S) {
  ...
  if (core) {
    var k = monsterDrawScale(m, S && S.currentZone);   /* v2.3.2786 */
...
function _pickLanding(S, a, m, tipX, tipY) {
  ...
  var ly = my - torsoLift(m, S && S.currentZone) + _tri() * core[1];   /* v2.3.2786: round the torso */
...
/* ═══ v2.3.2786: THE CIRCLE A SHOT MUST TOUCH SITS WHERE IT WAS AIMED ═══
...
/* A hit, on screen, at the point (tx, ty) where the shot landed: the monster's
   recoil and flash (v2.3.2200), its material (v2.3.2784), the sound of what was
...
function _projImpactFx(S, a, m, fx, tx, ty) {
  ...
  /* v2.3.2784: AN ARROW PUNCHES, A BOLT BLASTS -- the material leaves from
  ...
  if (fx.stub) {
    if (fx.snap) {
      /* v2.3.2731: one arrow in eight SNAPS instead -- the break is drawn at
         the arrow's pivot (queueArrowSnap), which lands a head-length behind
         its tip */
      var fr = _projBody(a).front;
      queueArrowSnap(S, tx - Math.cos(a.ang) * fr, ty - Math.sin(a.ang) * fr,
        (typeof m.renderY === 'number') ? m.renderY : m.y, a.ang);
    } else {
      /* v2.3.2511: one arrow, not two -- a plain arrow leaves its shaft, pinned
...
function _quarryInBox(S, a, b) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    var vx = fx - px, vy = (fy - _projCentreLift(S, a, m)) - py;   /* v2.3.2786: the hit test's own centre */
...
export function firstSightHit(S, ox, oy, ang, maxLen, opts) {
  ...
  if (list) {
    for (var i = 0; i < list.length; i++) {
      ...
      /* v2.3.2786: the monster the shot is aimed at, round its torso -- as the hit test will */
...
import { addBuildUse, applyMeleeLifesteal, distributeKillXpToBuild, trackMonsterDamage, pushDmgPopup, monsterPopupY, hurtPlayerLocal, isAttackInShieldArc, lockShotPoint, torsoLift, monsterDrawScale /* v2.3.2786: aimed at, and landing round, the torso */, spawnHitDebris /* v2.3.2200; v2.3.2784: its decal twin is retired here */, dropLocalRemnantOnce /* v2.3.2233 */, orbCrashFx, spawnPropDebris, propImpactSound, markProp /* v2.3.2730 */, queueArrowSnap /* v2.3.2731 */ } from '@/game/combatHelpers.js';
  ...
  var setRpgState = deps.setRpgState,
    setLevelUpMsg = deps.setLevelUpMsg;
        ...
        if (S.arrows && S.arrows.length > 0) {
          ...
          /* v2.3.2786: ...at the TORSO (lockShotPoint), and the target's hit circle is centred there for this shot (_projCentreLift) */
          var lockPt = lockShotPoint(S.lockedTarget && S.lockedTarget.ref, S.currentZone);
          var lockId = (lockPt && S.lockedTarget.ref.id != null) ? S.lockedTarget.ref.id : null;   /* v2.3.2786: see _projCentreLift */
          ...
          S.arrows = S.arrows.filter(function (a) {
            ...
            if (a.stuckIn) {
              ...
              if (_sAge >= 4000 || !_sm || !_sm.alive || _sm.curHp <= 0) {
                /* v2.3.2785: a monster that died while the arrow was still
              ...
              var _smy = ((typeof _sm.renderY === 'number') ? _sm.renderY : _sm.y) - torsoLift(_sm, S.currentZone);   /* v2.3.2786: the torso */
              var _stX = _smx + (a._stickOx || 0), _stY = _smy + (a._stickOy || 0);
              if (a._landFx) {
                /* ═══ v2.3.2785: THE SPECIAL FLIES IN, THEN LANDS ═══
                   ...
                   arrow's did (v2.3.2784).  Its chip ticks were already
            ...
            /* ═══ v2.3.2785: LANDING -- the shot has hit, and flies on in ═══
            ...
            if (S.monsters) S.monsters.forEach(function (m) {
              ...
              var _mProjY = _hitBaseY - _projCentreLift(S, a, m);   /* v2.3.2786: round the torso, for the monster it was aimed at */
              ...
              if (_segD < _hitRE) {
                ...
                /* Hit-reaction (ranged variant) — mirrors the melee path.
                   ═══ v2.3.2785: THE PICTURE WAITS FOR THE SHOT TO LAND ═══
                   The recoil, the white flash, the material burst (v2.3.2784)
                ...
                /* ═══ v2.3.2511: AN ARROW SOUNDS LIKE WHAT IT HIT ═══
                   ...
                   v2.3.2785: played by _projImpactFx now, when the shot LANDS in
                   the body, so the sound and the burst are one event. */
                var _hitMat = hitMaterialOf(m.archetype || m.type);
                /* v2.3.2785: the "blood spray on bow hits" is retired -- seven flat
                   ...
                   generic sprays (v2.3.2784 retired the splinters, v2.3.2782 the
                ...
                /* Magic orb crash & dissipate — element-tinted impact ring
                   ...
                   v2.3.2785: drawn by _projImpactFx where the orb LANDS; the
                ...
                if (a.isStaff) {
                   ...
                   * one that did not. */
                  /* ═══ v2.3.2785: ...AND NOW THE ORB IS IN THE BODY WHEN IT DOES ═══
                ...
                /* v2.3.2801: a staff bolt's hit is drawn once, by its crash above --
                   ...
                   v2.3.2784: and the arrow's generic splinter-and-dust spray
                ...
                /* Staff projectiles are magic — no physical shaft to
                   ...
                   crash above (v2.3.2782), and the material reaction (v2.3.2784). */
                ...
                /* ═══ v2.3.2785: THE SHOT FLIES ON INTO THE BODY, AND LANDS THERE ═══
                ...
                /* ═══ v2.3.2731: ...OR IT SNAPS ═══
                   ...
                   broken one could not.  v2.3.2785: rolled here, on the hit,
                ...
                if (!a.isStaff && a.isSpecial && !a.stuckIn && m.alive && m.curHp > 0) {
                  ...
                  /* v2.3.2785: ...give or take the body's core, so two specials
            ...
            /* v2.3.2785: ...and a shot flying on into the body it hit survives
            ...
            if (a._land) return true;   /* v2.3.2785: no prop stop for a shot that has already hit */
