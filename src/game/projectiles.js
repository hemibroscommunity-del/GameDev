...
/* v2.3.2715: the one-bolt staff special is the basic bolt's art drawn
...
export function monsterProjRadius(m, S, opts) {
  ...
  /* v2.3.2715: ...but not the one-bolt staff special (`big`).  Its reach is
...
function _projBody(a) {
  if (a.big) return PROJ_BODY.magicBig;   /* v2.3.2715: before the special row -- it is drawn as a bolt */
...
import {
  ...
  STAFF_BIG_BOLT_SCALE, /* v2.3.2715: the one-bolt special's drawn + hit size */
} from '@/data/index.js';
              ...
              if (_segD < _hitRE) {
                ...
                if (S._serverMonsters && S.channel) {
                  S.channel.send({ type: 'monster_damage', payload: {
                    ...
                    /* v2.3.2715: the one-bolt staff special carries the three
                ...
                if (a.isStaff) {
                  ...
                  /* v2.3.2730: the rings and the spray moved into combatHelpers'
                     ...
                     too, so props get it as well.  v2.3.2715: the one-bolt special's
