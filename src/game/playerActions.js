...
export function specialAttack(S) {
    ...
    if (activeWpn.type === 'bow') {
      ...
      if (S.channel) {
        for (var bvj = 0; bvj < _bowN; bvj++) {
          S.channel.send({ type: 'broadcast', event: 'player_projectile', payload: {
            id: S.myId, x: Math.round(S.player.x), y: Math.round(S.player.y), ang: aimAng, isStaff: false, isSpecial: true, ts: now, el: hasElement || undefined,   /* v2.3.2901: the element these shots are drawn in (hasElement above: the second element first); see monsterCombat's player_projectile */
      ...
      if (S._serverCaps && S._serverCaps.bigorb) {
        ...
        if (S.channel) {
          S.channel.send({ type: 'broadcast', event: 'player_projectile', payload: {
            id: S.myId, x: Math.round(S.player.x), y: Math.round(S.player.y), ang: aimAng, isStaff: true, isSpecial: true, el: hasElement || undefined,   /* v2.3.2901: the element these shots are drawn in (hasElement above: the second element first); see monsterCombat's player_projectile */
