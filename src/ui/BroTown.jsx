...
import { standFootDy } from '@/rendering/systems/entityRenderer.js'; /* v2.3.2846: the campfire is lit at your boots */
...
import { SPECIES_CATALOG, getSpecies, setSpecies, speciesDefaultSkin } from '@/rendering/traits/speciesCatalog.js';   /* v2.3.2682 */
...
import { triggerContextualDodge, dodgeWindowMs } from '@/game/dodge.js';   /* v2.3.2898: + the roll window, shared with the broadcast */
...
function nodeWorldBox(S, n) {
  ...
  if (spr && !spr.destroyed && spr.width > 2 && spr.height > 2) {
    ...
    /* v2.3.2846: the pixel campfire (rendering/campfireFx.js) on a flat zone:
...
export var BroTown = function BroTown(_ref0) {
  ...
  window._gameFns = {
    ...
    setSpecies: setSpecies,   /* v2.3.2682 */
    ...
    SPECIES_CATALOG: SPECIES_CATALOG,   /* v2.3.2682 */
    addLifeSkillXp: addLifeSkillXp,
    awardSkillXp: awardSkillXp,
    createMonster: createMonster,
    /* v2.3.2843: warm a zone's monster art from wherever the player stands,
    ...
    monsterTorsoY: DATA.monsterTorsoY,   /* v2.3.2845: where a locked shot is aimed and lands (mp-shotland) */
  ...
  /* v2.3.2682: the species, the same store-mirror pair. */
  var _spSelState = useState(getSpecies()),
    speciesSel = _spSelState[0],
    setSpeciesSel = _spSelState[1];
  ...
  /* v2.3.2424: the eyewear colour, threaded exactly like the hat's. */
  var _eyewearColorSelState = useState(getEyewearColor()),
    eyewearColorSel = _eyewearColorSelState[0],
    setEyewearColorSel = _eyewearColorSelState[1];
  ...
  }, [showNameModal, previewDir, skinSel, pantsSel, shoesSel, hairSel, hairColorSel, facialHairSel, beardColorSel, headwearSel, hatColorSel, shirtSel, shirtColorSel, eyeColorSel, eyeStyleSel /* v2.3.2643 */, speciesSel /* v2.3.2682 */, eyewearSel, eyewearColorSel /* v2.3.2424 */, heightSel, frameSel, activeCat, previewZoom]);
  ...
  var _thumbTile = function (cat, opt, selId, onSet, size) {
    ...
    return /*#__PURE__*/React.createElement("button", {
      ...
      ? /*#__PURE__*/React.createElement("div", { style: { width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' } },
          ...
          /*#__PURE__*/React.createElement("span", { style: { fontSize: 11, fontWeight: 700, letterSpacing: '.06em', color: '#3a4450', lineHeight: 1 } }, cat === 'species' ? opt.name : "None"))   /* v2.3.2682: on the species strip 'none' is the Human */
  ...
  var randomizeAppearance = function () {
    ...
    /* v2.3.2361: eyewear rolls with the rest.  While the catalog holds only
    ...
    /* v2.3.2424: and its colour, from what THIS pair offers -- eyewearColorsFor
       ...
       the hat roll two lines up already avoids. */
    if (recolorEnabled('eyewear')) {
      var _ewCols = eyewearColorsFor(ew);
  ...
  var resetLook = function () {
    ...
    setSpecies('none'); setSpeciesSel('none');   /* v2.3.2682: reset is the plain bro */
    ...
    var _gameLoop = function gameLoop() {
      ...
      try {
        ...
        if (S._dodgeRoll && !_playerDead) {
          ...
          /* v2.3.2898: the formula moved to game/dodge.js dodgeWindowMs so the
        ...
        {
          var _lbl = rLabelRef.current, _ring = rRingRef.current;
          ...
          /* ═══ v2.3.2760: THE CUE -- A MINI TOOL, STILL AND FLASHING UNTIL YOU MOVE ═══
             ...
             tool being used."
             gestureCueFace (gesturePose.js) owns every number; this only stamps
             them.  The phase is the CHASED value (_posF) the renderer has
             already written this frame -- the same number the character's
             frames come from -- so the tool under the thumb and the body move
             as one.  Until the first stroke it is 0 and the tool waits at the
             start of its track (idle: flashing, chevrons, a comet running the
             motion).  Every attribute write is guarded against its own last
             value: this runs every frame of every harvest, and a setAttribute
             on an SVG is a style invalidation even when the string is the same
             (the v2.3.2330 reason the ring caches its radius). */
          var _gPhase = null;
          if (_ex && _ex.status === 'ready') {
            _gPhase = Math.max(0, Math.min(0.9999,
              (_ex._posF != null) ? _ex._posF : (_ex.cueFrame01 || 0)));
          }
          if (_hint) {
            var _face = _gPhase != null ? gestureCueFace(_ex.skill, _gPhase, gestureIdle(_ex), Date.now()) : null;
            if (_face) {
              /* Addressed by data-cue, not by child index (v2.3.2384: the
                 track is drawn twice, dark under light). */
              if (_hint._btD !== _face.track) {
                var _trks = _hint.querySelectorAll('[data-cue="track"]');
                for (var _ti = 0; _ti < _trks.length; _ti++) _trks[_ti].setAttribute('d', _face.track);
                var _arws = _hint.querySelectorAll('[data-cue="arrows"]');
                for (var _ai = 0; _ai < _arws.length; _ai++) _arws[_ai].setAttribute('d', _face.arrows);
                _hint._btD = _face.track;
              }
              /* The sprite: which image, and which window of it, per skill. */
              var _spr = GESTURE_CUE_SPRITES[_ex.skill];
              if (_spr && _hint._btSkill !== _ex.skill) {
                var _sv = _hint.querySelector('[data-cue="sprite"]');
                var _im = _sv && _sv.firstChild;
                if (_sv && _im) {
                  _sv.setAttribute('viewBox', _spr.vb);
                  _im.setAttribute('href', _spr.url);
                  /* ...and the xlink spelling too: older iOS WebKit honours only
                     xlink:href on an SVG <image>, and iPhone Safari is the
                     primary platform (CLAUDE.md). */
                  try { _im.setAttributeNS('http://www.w3.org/1999/xlink', 'xlink:href', _spr.url); } catch (e) { /* ignore */ }
                  _im.setAttribute('width', String(_spr.w));
                  _im.setAttribute('height', String(_spr.h));
                }
                _hint._btSkill = _ex.skill;
              }
              var _tool = _hint.querySelector('[data-cue="tool"]');
              if (_tool) {
                var _tf = 'translate(' + _face.tool.x.toFixed(1) + ' ' + _face.tool.y.toFixed(1) + ') rotate(' + _face.tool.deg.toFixed(0) + ')';
                if (_tool._btTf !== _tf) { _tool.setAttribute('transform', _tf); _tool._btTf = _tf; }
                var _top = _face.tool.op.toFixed(2);
                if (_tool._btOp !== _top) { _tool.setAttribute('opacity', _top); _tool._btOp = _top; }
                var _glw = _tool.firstChild;
                var _gop = _face.glow.toFixed(2);
                if (_glw && _glw._btOp !== _gop) { _glw.setAttribute('opacity', _gop); _glw._btOp = _gop; }
              }
              /* The chevrons: bright while teaching, a faint guide once you go. */
              var _aop = _face.idle ? '0.95' : '0.35';
              if (_hint._btAop !== _aop) {
                var _arws2 = _hint.querySelectorAll('[data-cue="arrows"]');
                for (var _aj = 0; _aj < _arws2.length; _aj++) _arws2[_aj].setAttribute('opacity', _aop);
                _hint._btAop = _aop;
              }
              var _cmt = _hint.querySelector('[data-cue="comet"]');
              if (_cmt) {
                for (var _ci = 0; _ci < _cmt.childNodes.length; _ci++) {
                  var _dot = _cmt.childNodes[_ci];
                  var _cd = _face.comet && _face.comet[_ci];
                  if (_cd) {
                    _dot.setAttribute('cx', _cd.x.toFixed(1));
                    _dot.setAttribute('cy', _cd.y.toFixed(1));
                    _dot.setAttribute('opacity', _cd.op.toFixed(2));
                  } else if (_dot.getAttribute('opacity') !== '0') _dot.setAttribute('opacity', '0');
                }
              }
          ...
          /* ═══ v2.3.2472: A FOURTH STATE -- OUTLINE, WHILE A MONSTER IS UNDER IT ═══
             Owner (C1, backlog §2.4): the attack disc should ghost to an
             outline when a monster is actually behind it.

             v2.3.2263 already fades the disc's metal to 0.45 whenever `_hot`,
             which is "a monster is in play" -- a proxy, and a coarse one: the
             button dims through whole fights in which it is covering nothing
             but grass, and it still hides the one slime that walks under it.
             This is the real question instead, asked per frame: does any LIVE
             monster project inside the disc's box?

             THE PROJECTION IS THE RENDERER'S OWN, not a re-derivation:
             `screen = (world - camera) * S._worldScaleX/Y` (v2.3.1090's
             tap-to-lock, v2.3.1756's mouse aim -- both incidents were an
             inverse of this transform written slightly differently).  The body
             CENTRE is used rather than the feet, through the same
             DATA.monsterBodyY table the tap path uses, or a tall monster would
             stop counting while its head was still squarely over the button.

             MEASURED AGAINST THE CANVAS, not the viewport.  The disc is a fixed
             DOM element in viewport coordinates and the projection above lands
             in canvas coordinates; in LANDSCAPE the two differ by --world-x
             (game.css offsets .brotown-wrap), so subtracting the canvas's own
             rect is what makes this correct sideways as well as upright.  Both
             rects come from getBoundingClientRect, which is layout truth rather
             than an assumption about either offset -- and they are cached for
             400ms, because this runs every frame and a rect read forces layout.

             PAD: the box is grown by GHOST_PAD so a monster whose sprite laps
             onto the button counts before its centre does.  A blue slime's body
             is about 36px across on screen; half of that is the honest figure.

             It drops the painted METAL only.  The lit border, the label and the
             ring are siblings above it (v2.3.2263's layer split) and keep full
             strength, so the button still reads as a button -- this is the same
             rule v2.3.2251 and v2.3.2264 were both written to protect. */
          var _ghost = false;
          if (_rd && _rOn && S.monsters && S.camera) {
            var _grc = S._discRect;
            if (!_grc || _now2 - (S._discRectAt || 0) > 400) {
              S._discRectAt = _now2;
              var _drb = _rd.getBoundingClientRect();
              var _cvb = canvasRef.current ? canvasRef.current.getBoundingClientRect() : null;
              if (_drb.width > 1 && _cvb && _cvb.width > 1) {
                _grc = S._discRect = {
                  l: _drb.left - _cvb.left - GHOST_PAD, r: _drb.right - _cvb.left + GHOST_PAD,
                  t: _drb.top - _cvb.top - GHOST_PAD, b: _drb.bottom - _cvb.top + GHOST_PAD,
                };
              } else { _grc = S._discRect = null; }
            }
            if (_grc) {
              var _gsx = S._worldScaleX || 1, _gsy = S._worldScaleY || 1;
              var _gcx = S.camera.x, _gcy = S.camera.y;
              for (var _gi = 0; _gi < S.monsters.length; _gi++) {
                var _gm = S.monsters[_gi];
                if (!_gm || !_gm.alive) continue;
                if (typeof _gm.curHp === 'number' && _gm.curHp <= 0) continue;
                var _gwx = (typeof _gm.renderX === 'number' && isFinite(_gm.renderX)) ? _gm.renderX : _gm.x;
                if (typeof _gwx !== 'number' || !isFinite(_gwx)) continue;
                var _gpx = (_gwx - _gcx) * _gsx;
                if (_gpx < _grc.l || _gpx > _grc.r) continue;
                var _gpy = (DATA.monsterBodyY(_gm) - _gcy) * _gsy;
                if (_gpy >= _grc.t && _gpy <= _grc.b) { _ghost = true; break; }
              }
            }
          }
          S._discGhost = _ghost;   /* probe: __btDiscGhost, mp-joyfade / mp-btnlayout */
            ...
            window.__btDiscVis = function () {
              ...
              return { L: { shown: !!s2._lJoyShown, thumb: !!s2._lJoyHeld,
                       ...
                       R: { shown: !!s2._rBtnShown, liveMs: Math.max(0, (s2._rBtnLiveUntil || 0) - Date.now()),
                            ...
                            /* v2.3.2472: the ghosting rule -- whether a monster
                               is currently projecting under the disc, and the
                               canvas-space box it was tested against.  "The
                               disc is see-through" has two causes now (_hot and
                               this) and a screenshot cannot tell them apart. */
                            ghost: !!s2._discGhost, ghostRect: s2._discRect || null },
                       holds: discHoldProbe() };
            };
          }
        }
        ...
        if (S._firemaking && Date.now() >= S._firemaking.doneAt) {
          ...
          S._firemaking = null;
          /* ═══ v2.3.2846: THE FIRE IS LIT AT YOUR BOOTS, BESIDE YOU ═══
             ...
             boots are standFootDy() lower), so the fire stood 46 px behind your
             feet and was hidden by your own body until you walked off.  Now it
             is lit where the fire-lighting strip leaves its burning log: on the
             ground at your boots, a stride to your left, so you end the strike
             standing beside your fire rather than in it or in front of it. */
          var _cfZs = zonePlayerScale(S.currentZone, _fm.x, _fm.y - 6, TILE) || 1;
          var _cfX = _fm.x - 34 * _cfZs;
          var _cfY = (_fm.y - 6) + standFootDy(_cfZs) + 2 * _cfZs;
          S._campfire = {
            x: _cfX, y: _cfY, nodeType: 'campfire', alive: true,
          ...
          try {
            if (S.channel) S.channel.send({ type: 'broadcast', event: 'campfire_lit', payload: {
              id: S.myId, x: _cfX, y: _cfY, zone: S.currentZone, expiresAt: S._campfire.expiresAt,   /* v2.3.2846: the fire's own ground point */
            } });
          } catch (e) {}
        }
          ...
          if (S.channel) {
            ...
            if (S.channel && (!S._lastTrack || Date.now() - S._lastTrack > 2000)) {
              ...
              S.channel.track({
                ...
                sc: getSpecies(),   /* v2.3.2682: same road, same lesson */
                st: getShirt(),
                stc: getShirtColor(),
        ...
        if (S._shieldUp && S._shieldKb && typeof S._mouseAimAngle === 'number') {
          ...
          /* v2.3.2242: a raised shield faces the locked target every frame --
             BlockRing's lerp used to do this; the toggle button has no finger
             on it to steer, so the game is the only steer there is.
             ═══ v2.3.2472: ...AND WHEN THERE IS NO LOCK, THE NEAREST MONSTER ═══
             Owner decision D8: with the button back on bow and staff it
             "auto-aims at the nearest monster".  This branch used to REQUIRE a
             lock, which those two weapons almost never have (they acquire
             nothing automatically -- a tap is their only lock), so a raised
             guard kept whatever angle raiseShieldToggle seeded it with and
             never turned again as the fight moved.  The whole ladder lives in
             shieldAimAngle (lock -> nearest -> remembered -> aim -> facing), so
             calling it unconditionally is what hands those weapons the live
             tracking melee has had since v2.3.2242 -- and it is a no-op when
             there is nothing to point at, because the ladder then returns the
             angle already stored.
             v2.3.2451's `_rShieldSteered` term is gone with the hold gesture
             that set it: nothing on the sticks steers the arc any more. */
  ...
  var _desktopCycleWeapon = useCallback(function () {
    ...
    /* v2.3.2472: the guard SURVIVES the swap now.  v2.3.2446 dropped it on the
       way to a bow or a staff because it had just taken the shield button away
       on those weapons, so a raised guard would have stranded with nothing on
       screen to lower it.  D8 gives the button back on every weapon, so the
       hazard is gone -- and lowering a block because the player changed weapon
       is a behaviour nobody asked for: the owner's three exits from a guard are
       attack, tap again, and dodge. */
  ...
  var _desktopSelectSlot = useCallback(function (slot) {
    ...
    /* v2.3.2472: and the desktop's direct pick keeps the guard too -- same
       rule as the cycle above. */
  ...
  var handleJoystickMove = useCallback(function (clientX, clientY) {
    ...
    if (lStickRef.current) {
      ...
      /* v2.3.2317: the left rod takes the left disc's opacity for the same
         reason as the right -- see the note there. */
      var _lRodOp = (base && base.style && base.style.opacity) || '0.5';
      lStickRef.current.style.opacity = clampDist > 4 ? _lRodOp : '0';
    }
    var S = stateRef.current;
    var deadzone = 12;
    if (dist < deadzone) {
  ...
  var rJoyAim = useCallback(function (clientX, clientY, originX, originY) {
    ...
    if (rStickRef.current) {
      ...
      /* ═══ v2.3.2317: THE ROD WEARS THE ASSEMBLY'S OWN OPACITY ═══
         Owner: "the joystick rod isn't semi transparent like the rest of the
         joystick assembly."  It was not: the disc runs an engagement ladder
         (0.5 at rest, 0.92 with a finger down -- see bt-rjoy-base) while the
         rod carried a hard-coded 0.85 that tracked nothing. Two numbers for
         one object drift by construction, which is why it read as the solid
         part of a translucent control.
         The rod now takes whatever the DISC is currently wearing, so they
         cannot disagree again and a later retune of the ladder carries the
         rod with it for free. */
    ...
    /* ═══ v2.3.2472: THE HELD-GUARD GUARD IS GONE WITH THE HOLD ═══
       v2.3.2451 withheld the two attack flags here whenever `_rShieldHold` was
       set, because v2.3.2446 had made this very stick the shield's STEERING
       control and the drag that aimed the arc was re-arming autoAttack with no
       new press anywhere -- so monsterCombat's `if (S.autoAttack && S._shieldUp)
       dropShield(S, 'attack')` read a rotation as a request to attack and the
       guard fell the moment the thumb moved.

       D8 retires that hold, so the premise is gone: nothing steers the arc from
       this stick any more (shieldAimAngle points it at the lock, else the
       nearest monster), and a drag here means exactly what it meant before
       v2.3.2446 -- aim, and attack.  Breaking a raised guard is then the
       CORRECT reading of it: "attack" is the owner's own first-named exit from
       a block, and it is how the melee toggle has behaved since v2.3.2248. */
    S._aiming = true;
  ...
  var handleRBtnRelease = useCallback(function () {
    ...
    var S = stateRef.current;
    if (!S) return;
    /* v2.3.2472: nothing to end here any more -- the shield is a latching
       BUTTON on every weapon again, so a release on this side never lowers it.
       (v2.3.2446's `_endShieldHold()` call stood here.) */
  ...
  useEffect(function () {
    ...
    var rM = function rM(e) {
      ...
      if (t) {
        ...
        /* v2.3.2472: v2.3.2446's arc-steering block stood here (a raw atan2
           from the touch origin while `_rShieldHold` was set).  It went with the
           hold: the shield is a button again on every weapon and its arc comes
           from shieldAimAngle -- the lock, else the nearest monster. */
    ...
    var bM = function bM(e) {
      ...
      if (t) {
        ...
        /* v2.3.2472: and the disc's copy of that steer went with it. */
  ...
  useEffect(function () {
    ...
    try {
      ...
      if (tr.species != null) setSpecies(tr.species);   /* v2.3.2682 */
      ...
      if (tr.eyewearColor != null) setEyewearColor(tr.eyewearColor);   /* v2.3.2424 */
  ...
  if (showNameModal) {
    return /*#__PURE__*/React.createElement(NameModal, { onBack: backToMenu, /* v2.3.2219 */ _dragRotX: _dragRotX, _swatchTile: _swatchTile, _thumbTile: _thumbTile, _buildTile: _buildTile, activeCat: activeCat, heightSel: heightSel, setHeightSel: setHeightSel, frameSel: frameSel, setFrameSel: setFrameSel, beardColorSel: beardColorSel, facialHairSel: facialHairSel, hairColorSel: hairColorSel, hairSel: hairSel, hatColorSel: hatColorSel, eyeColorSel: eyeColorSel, setEyeColorSel: setEyeColorSel, headwearSel: headwearSel, eyewearSel: eyewearSel, setEyewearSel: setEyewearSel, /* v2.3.2361 */ eyeStyleSel: eyeStyleSel, setEyeStyleSel: setEyeStyleSel, /* v2.3.2643 */ speciesSel: speciesSel, setSpeciesSel: setSpeciesSel, /* v2.3.2682 */ eyewearColorSel: eyewearColorSel, setEyewearColorSel: setEyewearColorSel, /* v2.3.2424 */ joinTown: joinTown, nameInput: nameInput, pantsSel: pantsSel, previewCanvasRef: previewCanvasRef, previewDir: previewDir, previewZoom: previewZoom, setPreviewZoom: setPreviewZoom, randomizeWithFlair: randomizeWithFlair, resetLook: resetLook, rollRandomName: rollRandomName, rotatePreview: rotatePreview, setActiveCat: pickPreviewCat, setBeardColorSel: setBeardColorSel, setFacialHairSel: setFacialHairSel, setHairColorSel: setHairColorSel, setHairSel: setHairSel, setHatColorSel: setHatColorSel, setHeadwearSel: setHeadwearSel, setNameInput: setNameInput, setPantsSel: setPantsSel, setShirtColorSel: setShirtColorSel, setShirtSel: setShirtSel, setShoesSel: setShoesSel, setSkinSel: setSkinSel, shirtColorSel: shirtColorSel, shirtSel: shirtSel, shoesSel: shoesSel, skinSel: skinSel });
  }
