...
import { standFootDy } from '@/rendering/systems/entityRenderer.js'; /* v2.3.2718: the campfire is lit at your boots */
...
function nodeWorldBox(S, n) {
  ...
  if (spr && !spr.destroyed && spr.width > 2 && spr.height > 2) {
    ...
    /* v2.3.2718: the pixel campfire (rendering/campfireFx.js) on a flat zone:
    ...
    var _gameLoop = function gameLoop() {
      ...
      try {
        ...
        if (S._firemaking && Date.now() >= S._firemaking.doneAt) {
          ...
          S._firemaking = null;
          /* ═══ v2.3.2718: THE FIRE IS LIT AT YOUR BOOTS, BESIDE YOU ═══
          ...
          try {
            if (S.channel) S.channel.send({ type: 'broadcast', event: 'campfire_lit', payload: {
              id: S.myId, x: _cfX, y: _cfY, zone: S.currentZone, expiresAt: S._campfire.expiresAt,   /* v2.3.2718: the fire's own ground point */
            } });
          } catch (e) {}
        }
