    ...
    <ToastHost>
      <div onClick={() => { t.undos.forEach(u => u && u()); toastQueue.shift(); emitToast(); }}
        style={{
          ...
          background: '#FDFBF5', border: '1px solid rgba(34, 48, 60, 0.16)',
          borderRadius: 10, color: INV.textPrimary, fontFamily: FONT.sans, fontSize: 13,
          pointerEvents: 'auto', cursor: 'pointer',
          boxShadow: '0 4px 14px rgba(34,48,60,.25)',
        }}>{label}</div>
    </ToastHost>
  );
};
...
const Comparison = ({ item, layer2 }) => {
  ...
  return (
    <div style={{
      background: 'rgba(34, 48, 60, 0.05)', borderRadius: 8,
...
const GemsBlock = ({ item }) => {
  ...
  return (
    <div style={{
      background: 'rgba(34, 48, 60, 0.025)', borderRadius: 8,
      padding: '10px 12px', marginBottom: 12,
    }}>
      <div style={{
        fontSize: 10, color: 'rgba(34, 48, 60, 0.50)', letterSpacing: 0.4,
...
const ActionButton = ({ label, primary, destructive, onClick, ghost }) => {
  if (!label) return <div />;
  const bg = primary ? INV.primaryBtn : destructive ? INV.destructiveBg : 'rgba(34, 48, 60, 0.10)';
  const border = primary ? 'transparent' : destructive ? INV.destructiveBorder : 'rgba(34, 48, 60, 0.15)';
  const color = primary ? '#fff' : destructive ? INV.destructive : 'rgba(34, 48, 60, 0.85)';
  return (
      ...
      <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginBottom: 16 }}>
        {[0, 1].map(i => (
          <div key={i} onClick={() => onPick(i)} style={{
            ...
            border: '1.5px dashed rgba(34, 48, 60, .25)',
            background: 'rgba(34, 48, 60, .03)',
            display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
            cursor: 'pointer',
          }}>
            <div style={{ fontSize: 11, color: INV.textMuted, fontFamily: FONT.sans }}>SLOT {i + 1}</div>
            <div style={{ fontSize: 22, color: 'rgba(34, 48, 60, .4)' }}>+</div>
          </div>
        ))}
      </div>
    </div>
    ...
    <>
      ...
      <div onTouchStart={drag.onTouchStart} onTouchMove={drag.onTouchMove} onTouchEnd={drag.onTouchEnd}
        style={{
          ...
          background: INV.bg, borderTop: '0.5px solid rgba(34, 48, 60, 0.16)',
          borderTopLeftRadius: 14, borderTopRightRadius: 14,
        ...
        <div style={{ width: 32, height: 4, background: 'rgba(34, 48, 60, .2)', borderRadius: 2, margin: '0 auto 12px' }} />
        ...
        <div style={{ display: 'flex', gap: 14, alignItems: 'center', marginBottom: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontFamily: FONT.mono, fontSize: 12, color: 'rgba(34, 48, 60, .55)', marginTop: 3 }}>
              {subtitle}
            </div>
            {layer3 && item.crafter && (
