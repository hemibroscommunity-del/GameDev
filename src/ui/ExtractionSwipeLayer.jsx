...
import { GESTURE_STROKE, STROKE_SPAN_PX, GESTURE_FLOOR_MS, GESTURE_TARGET_MS, gestureTargetCycles } from '@/game/gesturePose.js'; /* v2.3.2748; GESTURE_TARGET_MS v2.3.2749 (the grade scales with it) */
 ...
 *
 * ═══ v2.3.2748: THE REP METER ABOVE IS NOW ~3s OF WORK ═══
...
/* px — travel past the last turning point to count a half-stroke.
   v2.3.2748: 40 -> 28.  The disc is 96px (108 landscape); a thumb pumping on
...
/* ═══ v2.3.2748: THE METER IS ABOUT THREE SECONDS AT A QUICK PACE ═══
...
/* The reward grade.  It used to be how early in a 3.5s window a 3-rep meter
   ...
   it (v2.3.2749: the owner doubled the target and these moved with it):
...
export const ExtractionSwipeLayer = ({ stateRef, onSuccess }) => {
  ...
  useEffect(() => {
    ...
    const beginGesture = (ex, x, y, cue, pointerId) => {
      ...
      if (!ex._gesture) {
        ex._gesture = {
          ...
          /* v2.3.2748: ONE stroke tracker drives both the pose phase and
      ...
      /* v2.3.2384: A THUMB THAT IS DOWN OWNS THE PHASE, FULL STOP.
         (v2.3.2748: the demo is gone from the body -- the flag now keeps the
    ...
    const onPointerMove = (e) => {
      const x = e.clientX, y = e.clientY;
      /* v2.3.2748: where the finger WAS, read before this move overwrites it
      ...
      if (ex.skill === 'fishing') {
        ...
        /* v2.3.2748: floored at 0 -- a player who winds the wrong way first
      ...
      /* ═══ v2.3.2748: THE PHASE THE BODY AND THE MINI TOOL PLAY ═══
    ...
    try {
      window.__btHarvest = () => {
        ...
        return {
          ...
          /* v2.3.2748: the three-second meter's parts -- cycles of motion
