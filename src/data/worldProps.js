...
export function propAnimStrips() {
  return WORLD_PROPS.filter((p) => propIsPlaced(p) && propIsResident(p) && p.anim && p.anim.frames > 1)
    .map((p) => ({ id: p.id, sprite: p.sprite, frames: p.anim.frames, zone: p.zone }));   /* v2.3.2859: + zone, so town's strips free with town */
}

/* v2.3.2859: the resident props of ONE hub.  Town's load and free with town
