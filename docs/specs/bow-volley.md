...
| Spacing | — | 200 px tip to tip (one arrow is 62.8), on one line — v2.3.2875, was 80 |
| Speed | — | 0.7 × the bow's own (`SPEED_K`, set as each arrow's `speedPx`) — v2.3.2875 |
...
| `player_projectile.delayMs` | client ↔ client relay | 0 / 417 / 833 per arrow (v2.3.2875; was 0 / 167 / 333) | The staff volley's field (v2.3.2259). A peer's arrow flies 8 px a frame, so the stagger is sized for that, and a peer holds a non-staff projectile that many **frames** (`holdFrames`), not milliseconds — in milliseconds the gap shrank with the watcher's frame rate (measured 14–22 px apart on a busy tab). |
