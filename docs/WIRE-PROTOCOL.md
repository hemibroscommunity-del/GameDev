...
| `monster_damage` | Damage claim against a monster (server validates; truth returns as `monster_hit`). **v2.3.2481:** a BASIC staff bolt (`slot: 'staff'`, no `special`) additionally splashes 50% of its own capped roll to up to 3 other monsters within 60 px of the impact point (× Detonation, the server mirror of the client's `staffAoeMult`), each answered by its own `monster_hit` tagged `splash: true`. The scan is server-side ON PURPOSE: the worker rolls FULL damage for every `monster_damage` it receives, so a client sending a second event for the neighbour would deal 100%, not 50%. **v2.3.2783: `orbs`** — the staff special is ONE big bolt (behind `caps.bigOrb`) and says how many of the old volley's orbs it stands for (`orbs: 3`). Honoured only on a staff special (`slot: 'staff'`, `special: true`), clamped to 1..3, and each orb spends a slot in the special lane (3 per 1200 ms per monster); the worker rolls that many special hits, each under its own cap, and sums them into ONE `monster_hit`. Absent -> one roll, byte-identical | ~3834 |
