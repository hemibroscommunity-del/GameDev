#!/usr/bin/env python3
"""v2.3.1311: slice the owner's hero-stat icon sheet.

Input:  assets/icons-source/sheet-hero-stats.png
        (4x4 grid on MAGENTA, a text label baked under each icon)
Output: public/icons/ui/hero/<name>.webp  (256x256 RGBA, transparent)

Same pipeline as process_welcome_cc_sheet.py (v2.3.1307): global
magenta chroma key (so enclosed regions clear too), despill fade on
the AA fringe, label band dropped by keeping the tallest vertical
content band per cell, widest horizontal band to shrug off neighbor
slivers.  Only the manifest differs.
"""
import os
from PIL import Image

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
SRC = os.path.join(ROOT, 'assets', 'icons-source', 'sheet-hero-stats.png')
OUT_DIR = os.path.join(ROOT, 'public', 'icons', 'ui', 'hero')
OUT_SIZE = 256
MARGIN = 0.12

# Row-major, matching the sheet's baked labels.  NOTE the engine keeps
# its storage key 'endurance' for the sixth combat parent, but the
# owner's canonical NAME for that parent is Stamina; the boot art
# labeled ENDURANCE on the sheet is that parent's icon (the bolt is
# the stamina RESOURCE icon used on the vitals bar).
NAMES = [
    'hp', 'stamina', 'mana', 'xp',
    'melee', 'bow', 'magic', 'vitality',
    'defense', 'endurance', 'damage', 'dps',
    'damage-reduction', 'crit', 'dodge', 'move-speed',
]
COLS, ROWS = 4, 4

# Per-name vertical band-split gap override.  The default 10px split
# cleanly drops every baked label except CRIT's, whose starburst tips
# reach to within <10px of the text — a tighter gap separates them.
VGAP = {'crit': 4}
