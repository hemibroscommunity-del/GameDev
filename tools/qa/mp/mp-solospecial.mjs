  ...
  window.__soloIv = setInterval(() => {
    ...
    for (const a of (S2.arrows || [])) {
      ...
      window.__solo.seen.push({
        ...
        big: !!a.big, orbs: a.orbs || 0,   /* v2.3.2741: the one-bolt special */
...
export async function run({ browser, wsPort, webPort, rec }) {
  ...
  /* ═══ v2.3.2741: THE MAGIC SPECIAL IS ONE BIG BOLT, THE VOLLEY IS LEGACY ═══
  ...
  /* v2.3.2741: the big bolt carries the volley it replaced. */
  ...
  for (const w of [
    { key: 'bow',   type: 'bow',   stash: 'rangedWeapon', slot: 'ranged', own: 1 },
    { key: 'magic', type: 'staff', stash: 'staffWeapon',  slot: 'staff',  own: 1 },   /* v2.3.2741: one big bolt */
  ]) {
    await H.equipWeapon(P, w.type, w.stash, w.slot);
