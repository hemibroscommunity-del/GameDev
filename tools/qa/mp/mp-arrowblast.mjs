 ...
 *
 * ═══ v2.3.2808: NOW THE KILL SWITCH'S TEST ═══
...
async function blast({ browser, wsPort, webPort, rec }) {
  ...
  await A.page.evaluate(({ tx, ty, id }) => {
    ...
    S._facing = 'right';
    /* v2.3.2808: LOCKED, the way a tap does.  Since v2.3.2473 a bow special
  ...
  rec.ok('...and it is the ONE arrow of old, not the volley (v2.3.2808)', live.length === 1, live);
  ...
  rec.ok('the kill switch let the blast past the retired gate (v2.3.2808: no "retired" refusal)',
