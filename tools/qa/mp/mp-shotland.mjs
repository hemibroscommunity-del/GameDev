/* A SHOT LANDS IN THE BODY, NOT ON AN INVISIBLE EDGE (v2.3.2785).
 *
 ...
 * each.
 *
 * v2.3.2786 -- AT THE TORSO.  Owner: "The arrows are grouping around the
...
/* LAND_CORE (projectiles.js): the half-width and half-height of the region a
   shot lands in, round the torso (v2.3.2786; the body centre before).  `r` is
...
const shootOnce = (P, wpn) => P.page.evaluate((wpn) => new Promise((resolve) => {
  ...
  const hp0 = m.curHp;
  /* v2.3.2785: one arrow in eight SNAPS (v2.3.2731) instead of leaving a
     shaft -- it lands just the same, and breaks where it lands */
  const t00 = performance.now();
  /* v2.3.2786: the centre a shot lands round is the torso now */
  ...
  const tick = () => {
    ...
    for (const a of arrows) {
      if (!shot) shot = a;
      if (a !== shot) continue;
      /* v2.3.2786: the height its flight line crosses the monster's x at, read
...
export async function run({ browser, wsPort, webPort, rec }) {
  ...
  for (const mon of MONS) {
    for (const wpn of WEAPONS) {
      ...
      if (wpn === WEAPONS[0]) {
        /* v2.3.2786: the torso is where the owner asked for it, and inside the
      ...
      /* v2.3.2786: the flight line goes through the torso, and the landings sit on it */
  ...
  /* ── v2.3.2786: the room a torso shot has to hit is the room it always had ──
