...
export const fireTrailMethods = {
  ...
  _maybeDropFirePatch(zoneId, m, now) {
    ...
    const patch = {
      mid: m.id,
      lvl: m.level, /* v2.3.2659: the goblin's level, so Resist's edge is priced against whoever lit it */
      x: m.x, y: m.y,
  ...
  _fireTrailHitPlayer(zoneId, patch, pid) {
    ...
    const res = this._applyDamage(ps, raw, false, { elemental: true, attackerLevel: patch.lvl });  /* v2.3.2659: + the edge */
