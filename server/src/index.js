...
export const TRACK_COSMETIC_KEYS = new Set([
  ...
  /* v2.3.2428: +`pb`, the back-of-trousers drawing -- see join.js. */
...
export class GameRoom {
  ...
  _pickSpreadSpawn(zone, placed, avoid, avoidClearPx) {
    ...
    /* v2.3.2894: a zone's arrival point (data.js `entryClear`) outranks
  ...
  _tickMonsters() {
    ...
    for (const zoneId of activeZones) {
      ...
      for (const m of monsters) {
        ...
        /* ═══ v2.3.2790: ...TIMES THE ZONE'S DEPTH AT THIS MONSTER'S FEET ═══
        ...
        /* v2.3.2790: a far monster notices you as far off as it LOOKS (x _dk).
        ...
        if (nearest && nearestDist < effAggroRange) {
          ...
          if (attackDist > _atkRange && !attackingNow && ccMoveMult > 0) {
            ...
            if (dist > 0) {
              ...
              let step = m.spd * ccMoveMult * _dk;   /* v2.3.2790: slower where it is drawn smaller */
              if (m._kbDebt > 0) {
          ...
          const _rangedBase = Object.prototype.hasOwnProperty.call(this.MONSTER_RANGED_BY_ARCH, m.arch)
            ...
            : null;
          /* v2.3.2790: the throw band shrinks with the thrower (x _dk) -- the
          ...
          /* v2.3.1639: drop any unpaid knockback debt on aggro loss, so a
             ...
             its next engagement. */
          m._kbDebt = 0;
          /* v2.3.2790: a wandering far monster strolls as far as it looks
      ...
      // push-out in BroTown.jsx) so the server never shoves a player.
      {
        ...
        for (let i = 0; i < monsters.length; i++) {
          ...
          for (let j = i + 1; j < monsters.length; j++) {
            ...
            /* v2.3.2790: two far monsters drawn at 0.42 stand as close as
  ...
  _handleLootPickup(session, payload) {
    ...
    if (pile.armor && pile.armor.length && !pile.armorClaimed) {
      ...
      /* ═══ v2.3.2534: THE MINT IS RECORDED HERE, NOT AT THE KILL ═══
         ...
         provenance is written at the moment ownership is decided, which is
         this one — `gear_prov:<playerId>` names the player the row belongs
         to, and that has to be a player (gearprov.js).

         Each piece is COPIED before it is stamped.  The pile's own objects
         stay unmarked because _publicPile still maps them through
         _mysteryPiece to every watcher in the zone, and a gid has no
         business on a room-wide broadcast.  drops.test.mjs already pins
         that the public pile never carries `quality`; the same crop is what
         keeps it from carrying `gid`. */
      armorForMe = pile.armor.map((a) => this._gearProvRecord(
        session.id, (a && a.slot === 'legsArmor') ? 'legsArmor' : 'armor', { ...a }, 'drop'));
    }
    pile.claimedBy[session.id] = true;
