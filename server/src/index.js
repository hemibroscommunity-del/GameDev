...
// v2.3.1146: live-ops rail -- flags/kill-switches, announcements/MOTD,
// daily economy metrics (see liveops.js header).
import { LIVEOPS, liveopsMethods } from './liveops.js';
// v2.3.1148: save-format migration registry (run in _loadRpg; _saveRpg
...
export default {
  async fetch(request, env) {
    ...
    // v2.3.1144: operator toolkit -- routes into the GameRoom that owns
...
export const PRIVILEGED_EVENTS = new Set([
  ...
  // v2.3.1146: operator announcements -- deliberately NOT riding the
...
export class GameRoom {
  ...
  async _loadRpg(playerId) {
    try {
      ...
      // v2.3.1148: run-once migration registry replaces the every-load
  ...
  // multiplayer corruption storm.  v2.3.1148: body moved to the pure
  ...
  async _saveRpg(playerId, ps) {
    ...
    try {
      await this.state.storage.put('rpg:' + playerId, {
        ...
        // v2.3.1148: schema stamp -- the ONE field allowed beyond the
  ...
  _spawnLootForKill(zone, monster, killerSessionId, recipients, shares) {
    ...
    // v2.3.1146: disable_weapon_drops kill switch -- caps.weaponDrops
  ...
  _resolveMonsterKill(zone, m, killerId, killerPs, slot) {
      ...
      for (const rid of xpRecipients) {
        ...
        // v2.3.1146: xp_mult live-ops flag -- the "2x weekend" lever.
  ...
  async fetch(request) {
    ...
    // v2.3.1144: operator toolkit (owner-keyed; fail-closed without the
    ...
    switch (msg.type) {
      case 'join':
        ...
        if (msg.id) {
          ...
          // v2.3.1144: operator freeze gate.  Storage await keeps the
           ...
           value. */
        {
          ...
          // v2.3.1144: lazy daily snapshot of the PRE-join blob (the
          ...
          if (stored) {
            ...
            // v2.3.1148: the every-load strip moved to migration v2
            ...
            // v2.3.1148: boundary heal.  Migration v1 fixes STORED
        ...
        // v2.3.1146: sticky MOTD delivery + the lazy daily economy
        ...
        // v2.3.1146: warm the live-ops flag cache before anything gated
        ...
        ws.send(JSON.stringify({
          ...
          caps: { trade: true, questTrack: true, gamble: true, clans: true, arena: true, dungeon: true, sponsor: true, guilds: true, pets: true, harden: true, trade2: true, weaponDrops: true, jackpot: true, ..._liveFlags },
  ...
  startTickLoop() {
    ...
    this.tickInterval = setInterval(() => {
      ...
      // week boundary.  v2.3.1146: the daily metrics snapshot rides
...
// v2.3.1146: live-ops rail -- see liveops.js.
