...
export const arrowBlastMethods = {
  /* ═══ v2.3.2848: RETIRED WHILE THE BOW SPECIAL IS A VOLLEY ═══
  ...
  _arrowBlastRefusal(ps, zone, x, y, now) {
    if (this._bowVolleyLive()) return 'retired';   /* v2.3.2848: see _bowVolleyLive */
  ...
  _handleArrowBlast(session, payload) {
    ...
    for (const m of monsters) {
      ...
      const rolled = this._computeAttackDamage(ps, 'ranged', true, { targetLevel: m.level });  /* v2.3.2680: the edge */
