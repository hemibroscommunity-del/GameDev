...
export const telegraphMethods = {
  ...
  _resolveMonsterTelegraph(zoneId, m, now) {
    ...
    if (!kit || m._tgPhase !== 'telegraph') return false;
    {
      ...
      const _tgR = m._tgRadius || kit.radius;   /* v2.3.2756: the ring it drew */
      m._tgRadius = 0;
      m._tgPhase = null;
      m._tgUntil = 0;
      m._tgAim = null;
  ...
  _maybeStartTelegraph(zoneId, m, nearest, attackDist, now) {
    ...
    /* v2.3.2756: the cast range and the ring shrink with the caster on a
    ...
    m._tgRadius = kit.radius * _dk;               /* v2.3.2756 */
    m._attackingUntil = Math.max(m._attackingUntil || 0, now + 100);
  ...
  /* v2.3.2756: `zoneId` (optional) folds in the zone's depth at the
  ...
  _startSlimeBurst(zoneId, m, killerId, slot, now) {
    ...
    /* v2.3.2756: the blast is as wide as the swollen body LOOKS -- x the
  ...
  _resolveSlimeBurst(zoneId, m, now) {
    ...
    const _bR = m._burstR || SLIME_BURST.RADIUS;   /* v2.3.2756 */
    const r2 = _bR * _bR;
