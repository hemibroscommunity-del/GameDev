...
export const JOIN_COSMETIC_KEYS = [
  ...
  'es',   /* v2.3.2643: the eye STYLE -- a short catalog id like hw/fh/ew, and a
  ...
  async _handleJoin(session, ws, msg) {
       ...
       value. */
    {
      ...
      // level * 10 + 20 -- bounded forever after, even on reconnect.
      {
        ...
        /* v2.3.2662: the join-time milestone settle (v2.3.1733) is gone
    ...
    /* v2.3.2619: Ace's hall of fame, on the same join push as the jackpot --
    ...
    ws.send(JSON.stringify({
      ...
      char: (session.char
        ...
        : null),
      caps: { charLock: true /* v2.3.1814: name+look are stored server-side and a stored record WINS over the join payload.  The client gates BOTH halves of the permanent-character flow on this — skipping the creator, and trusting state_sync.char over its local trait catalogs.  Against an OLD worker the flag is absent, no record is ever stored, and the client keeps its old behaviour of picking a look every session; against a NEW worker an old client simply sends a look and has it locked in on first join, which is the intended migration either way (deploy-order safety, rule 19). */, trade: true, questTrack: true, gamble: true, aceFlip: true /* v2.3.2618: Ace's coin flip (gamble.js).  Its own narrow flag, NOT a widening of `gamble`: a v2.3.1124 worker advertises the Hall and has no case for ace_flip_request, so it would fall to the default branch and REBROADCAST the stake to the room while settling nothing -- the player would watch a coin land and never be paid.  So the client gates its Flip button and its send on this, and against an old worker Ace simply says he is not taking bets today.  Against a NEW worker an old client never sends one (deploy-order safety, rule 19). */, aceItems: true /* v2.3.2619: the item wager (double or nothing) and the hall-of-fame board.  Its own flag rather than a widening of `aceFlip`: a v2.3.2618 worker advertises the coin flip and has no case for ace_item_flip_request or ace_board, so it would fall to the default branch and REBROADCAST a bag full of item keys to the room while settling nothing.  The client gates its Items tab, its Records tab and both sends on this; against an old worker Ace only takes gold (deploy-order safety, rule 19). */, clans: true, arena: true, dungeon: true, sponsor: true, guilds: true, pets: true, harden: true, trade2: true, weaponDrops: true, botfp: true, jackpot: true, hpEndGrids: true, t2uniform: true, httpAuth: true, party: true, amuletForge: true, gems: true, petLoot: true, gemExtract: true, partyChat: true, trade2Weapons: true, trade2Review: true /* v2.3.1754: the two-stage trade (ready -> review -> accept), its server-enforced accept cooldown, and the trade2_ready type.  The client gates its Ready button and its trade2_ready send on this: against an OLD worker the flag is absent, the window keeps the single-stage Confirm it has always had, and nothing is sent that the worker would relay as an unknown broadcast.  Against a NEW worker an old client simply never readies — and _handleTrade2Confirm would refuse it, so the pairing must be advertised (deploy-order safety, rule 19). */, laststand: true, friends: true /* v2.3.1323 */, t2simple: true /* v2.3.1342: level = T2 points placed (cap 1000); client gates its level derivation + spend celebration on this so an old worker's player_state echo can't stomp the new formula */, t2bench: true, broVerify: true /* v2.3.1576: Hemi Bro ownership. Gates the client's wallet control (broWallet.broVerifySupported) so it only appears against a worker that can settle it; an old client never sends the types. Safe in either deploy order (rule 19). */, prog3Chan: true /* v2.3.2176: points remember the combat skill that earned them (prog3.js poolBy).  The client gates its per-lane point counts AND its "you cannot buy Bow crit with a Melee point" affordance on this: against an OLD worker there is no poolBy, so the client shows the single shared total it always did and lets the server judge every spend as before.  Against a NEW worker an old client simply sends spends without naming a lane for body stats, and the worker picks the largest channel -- which is why that fallback exists (deploy-order safety, rule 19). */, prog3: true /* v2.3.1659: the trained-skill combat rebuild (prog3.js). The client gates its new Build UI, prog3_allocate sends, trained-level readouts, and the retirement of its local _buildProg/weapon-XP accrual on this flag — an old worker would relay prog3_allocate as an unknown type and its player_state echoes would stomp prog3-derived pools (deploy-order safety, rule 19). */, prog3x: true /* v2.3.2199: the 3-points economy expansion — POINTS_PER_LEVEL 3, the two new spendable stats (ATK dmg, BODY elem), and critDmg's flat→percent semantics.  The client gates DISPLAY on this, not sends: the new stat rows, the "+3 points" level banner, and the percent critDmg readout/DPS math.  Against an OLD worker the flag is absent, the rows hide, the banner says +1 and the DPS math predicts the flat +2 crits that worker actually rolls — nothing new is ever sent that it would drop.  Against a NEW worker an old client simply doesn't show the new rows; its prog3_allocate sends are unchanged and the worker accepts them (deploy-order safety, rule 19). */, prog3elem: true /* v2.3.2512: the attribute restructure -- elemental power moved from ONE global stat to an ATK channel PER combat type, and two new global stats arrived (Elem Resist, Max Mana).  The client gates DISPLAY on this, not sends: the three new stat rows, its elemental-power readouts (which keep reading the retired GLOBAL stat against an old worker, because that is what that worker actually rolls off) and its max-mana prediction (which keeps the pure Magic-level derivation an old worker computes).  Nothing new is ever SENT on this flag -- a prog3_allocate naming `elem` with a `cat` is shaped exactly like today's other per-type spends, and an old worker simply refuses it.  Against a NEW worker an old client never shows the rows and never spends on them (deploy-order safety, rule 19). */, prog3shared: true /* v2.3.2592: the four-column points redesign -- six stats per combat type (crit + critDmg folded into LUCK; RANGE and SPECIAL new), a seventh shared stat (MOVE SPEED), and a SECOND POOL: every level-up mints lane points AND shared points, and a lane point can no longer buy a shared stat.  The client gates DISPLAY on this, not sends: the new rows, the luck readouts and DPS math, and the per-column point counts (shared pool vs. lane pools).  Against an OLD worker the flag is absent, the client draws that worker's grid (crit/critDmg rows, no range/special/move) and its pools (lane points buy body stats), because that is what the worker will settle.  Nothing new is ever SENT: a prog3_allocate naming `luck` with a `cat` is shaped exactly like today's per-type spends and an old worker's whitelist refuses the name silently -- the same posture as a hidden row.  Against a NEW worker an old client keeps sending `cat` on a body spend, which _handleProg3Allocate now ignores (the point comes off the shared pool), and never offers a row it does not know (deploy-order safety, rule 19). */, prog3rel: true /* v2.3.2680: relative point value -- every curve stat (Power, Luck, Special, Element, Defense, Dodge, Resist, Move, Speed, Range) reads max x q/(q+k) instead of pts x per, the seven that change a hit fade against a stronger monster, Power multiplies instead of adding, the damage-stat per-level bound doubles, and an elemental hit meets Resist alone.  The client gates its READOUTS on this, not its sends: against an OLD worker it keeps predicting that worker's linear per-point math (PROG3_LINEAR in src/data/prog3.js) and its caps; against a NEW worker an old client just shows the old linear numbers while the worker rolls the new ones -- display drift only, since every prog3_allocate is shaped exactly as before and the worker is the authority either way (deploy-order safety, rule 19). */, gearq: true /* v2.3.2664: gear that matters -- a weapon's grade multiplies its WHOLE hit (normal 1 / rare 1.3 / elite 1.75 / godly 5, data.js QUALITY_GRADES) instead of its 10-point base, its tier factor is tierMult^1.5, armour becomes a five-tier ladder (+7.5 % / +5 % per whole tier) whose ceiling each piece's grade raises, and godly is 1 in 2,000,000.  The client gates its damage and armour READOUTS on this: against an OLD worker it keeps predicting that worker's math (QUALITY_MULTS_LEGACY on the base, tierMult straight, ARMOR_DR_LEGACY); against a NEW worker an old client just shows the old numbers while the worker rolls the new ones -- display drift only, nothing new is ever sent (deploy-order safety, rule 19). */, abil: true /* v2.3.1733: stamina abilities + the milestone unlock ladder (abilities.js). The client gates its ability BUTTONS and its `ability` sends on this, so against an old worker (which would relay the unknown type as a broadcast and never settle it) no button appears and nothing is sent — deploy-order safe in either order (rule 19). */, elemBurst: true /* v2.3.1734: Element Burst (burst.js). Gates the client's burst button, its desktop key and its element_burst send — against an old worker the button never appears and nothing is sent, and against a new worker an old client simply never casts it. ALSO gates the client's FLAT special-attack mana cost: the cost is charged by the worker (_abilityCost), so a new client against an old worker must keep predicting floor(maxMana/5) or its charge pie promises casts that worker will refuse (deploy-order safety, rule 19). */, blockScale: true /* v2.3.2302: the block COUNT grows with investment (5 at base, 10 fully invested; abilities.js blocksAt).  The client gates BOTH its cost prediction and its bar length on this: against an OLD worker the flag is absent, the client keeps predicting the FIFTHS that worker actually charges and keeps drawing five blocks, so the charge pie never promises a cast the worker refuses.  Against a NEW worker an old client simply keeps predicting fifths -- it under-counts its own casts but never over-promises, because the real cost only ever gets CHEAPER as the count grows (deploy-order safety, rule 19). */, areaChat: true /* v2.3.2136: zone-scoped chat (chatlanes.js).  The client gates its /a send on THIS narrow flag, not on any broader one: an older worker has no case for area_chat, so it would fall through to the default branch and REBROADCAST the line to the whole room -- the quiet lane made loud.  Against a NEW worker an old client simply never sends it (deploy-order safety, rule 19). */, whisper: true /* v2.3.2136: whisper-by-name (chatlanes.js).  Gated on its own flag for the same reason as areaChat and with worse stakes -- an un-upgraded worker would relay a PRIVATE line to everyone in the room.  So the client must never send /w to a worker that has not advertised this. */, chatMute: true /* v2.3.1981: server-side chat mute + reports (chatmod.js). The client gates its chat_mute/chat_report sends AND its "muted lines still arrive, shown as [muted]" fallback on this: against an OLD worker the flag is absent, nothing is sent (an unknown type would be REBROADCAST to the room -- a mute telling everyone who you muted), and the localStorage list keeps doing the filtering it always did. Against a NEW worker an old client simply keeps filtering locally over a stream the worker is already filtering, which is harmless (deploy-order safety, rule 19). */, potionBag: true /* v2.3.2127: potions are bought INTO the bag and drunk from it (shop.js _shopBuy staple branch, cooking.js _handleDrinkRequest). The client gates its Drink button and its potion_drink send on this: against an OLD worker a staple purchase still applies its effect on the spot, no bottle can be in the bag, and nothing is sent that the worker would relay to the room as an unknown broadcast. Against a NEW worker an old client simply buys a bottle it never learns to drink (deploy-order safety, rule 19). */, eventCapes: true /* v2.3.2026: the golden-ticket drop and cape_redeem (eventcapes.js). The client gates its Open button and its cape_redeem send on this: against an OLD worker the flag is absent, no ticket can exist to open, and nothing is sent that the worker would relay to the room as an unknown broadcast. Against a NEW worker an old client simply never opens a ticket it cannot have been given -- the drop is gated on the same event flag (deploy-order safety, rule 19). */, gearRef: true /* v2.3.2535: equipping NAMES a recorded piece instead of describing one (gearprov.js, gear-provenance.md).  The client gates its `armorRef`/`legsArmorRef` sends on this: against an OLD worker the flag is absent and it keeps sending the whole armour object on stats_update, which is what that worker understands -- a ref it could not resolve would be an unknown field and the piece would never be equipped at all.  Against a NEW worker an old client simply keeps describing, and the describe lane still accepts it (it has to: a piece minted before v2.3.2534 has no id to name).  Narrow on purpose and NOT folded into `store`: a v2.3.2475 worker advertises the store and knows nothing about refs (TRAPS #9, the caps.gems lesson).  Deploy-order safe both ways (rule 19). */, store: true /* v2.3.2475: the auction house (store.js) -- per-listing sales of stackables and stash weapons over /api/store.  The client gates its Sell button, its Store panel and its store fetches on this: against an OLD worker there is no /api/store route at all, so every call would 404 and the panel would show an empty shelf it could never fill -- worse than no button.  Against a NEW worker an old client simply never opens a store it does not know about (deploy-order safety, rule 19). */, storeGear: true /* v2.3.2531: GEAR listings in the auction house (storegear.js) -- armour, legs, shields, cosmetic layers and amulets, escrowed out of the server-side gear stashes that v2.3.2523 created.  Its own narrow flag rather than a widening of `caps.store`, for two separate reasons.  Deploy order (rule 19): against an OLD worker `kind: 'gear'` is an unknown listing kind and the request is refused, so a new client must not offer the Sell button on a gear card or the piece would look like it had vanished; against a NEW worker an old client simply never sends one.  And a KILL SWITCH: join.js spreads `..._liveFlags` last over the baked caps, so writing `storeGear: false` into the `liveflags` key un-advertises the surface AND (store.js `_stGearOff`) stops the route accepting a listing, with no deploy and without disturbing the rest of the store.  That lever is load-bearing here: being in a gear stash is not proof of ownership (gear-stash.md "What this does NOT solve"), and the risk was accepted deliberately rather than closed -- see storegear.js's header. */, storeGearRef: true /* v2.3.2551: a gear listing NAMES its piece by the server-assigned `gid` (gearprov.js) instead of describing it with a selector.  Its own narrow flag and deliberately NOT a widening of `caps.storeGear`, for the ordinary rule-19 reason plus the caps.gems lesson (TRAPS #9): a v2.3.2531 worker advertises `storeGear` and knows nothing about `gid` -- it would read `body.sel` off a request that carries none and answer "Invalid item", so a new client must keep describing until a worker says it can do better.  Against a NEW worker an old client's selector is still accepted and resolved against the server's own list, which it has to be: that is the path every browser is on until Pages redeploys.  The gate itself is NOT gated on this -- `_gearSellable` runs on both naming paths -- so the flag buys accuracy (an id reaches a minted piece our stash snapshot has not adopted yet), never permission.  Deploy-order safe both ways (rule 19). */, storeChat: true /* v2.3.2621: per-listing message threads in the auction house (storechat.js).  Its own narrow flag and MANDATORY, not a nicety: an older worker has no case for `store_dm` or `store_dm_open`, so both would fall through to the DEFAULT BRANCH and be REBROADCAST to the whole room -- a private haggle over a sword shouted at everybody, which is the same worst-case chatlanes.js argues for /w.  So a client must never send one to a worker that has not advertised this.  Against a NEW worker an old client simply never shows the chat icon (deploy-order safety, rule 19). */, milestonesRetired: true /* v2.3.2662: the milestone ladder is gone (abilities.js tombstone) -- no x1.25 max stamina at level 10, no level-6 gate on Element Burst.  The client gates its two PREDICTIONS on this, never a send: against an OLD worker the flag is absent and the client keeps predicting the x1.25 bar and hiding the Burst button below level 6, because that is what that worker still settles.  Against a NEW worker an old client over-predicts stamina at 10+ until the next player_state (which carries the server's maxStamina and wins) and hides a Burst button the worker would accept -- stale, never unsafe (deploy-order safety, rule 19). */, storeOffer: true /* v2.3.2623: escrowed offers on a listing (storeoffer.js) -- a buyer's gold is DEBITED when they offer and the seller's Yes settles on the spot.  Its own narrow flag and mandatory: an older worker has no case for `store_offer`, so the request would fall through to the default branch and be rebroadcast as chatter -- the buyer would see their offer 'sent', no gold would move, and no seller could ever accept it.  A money surface that silently does nothing is worse than an absent button, so the offer control must not exist against a worker that cannot settle it (deploy-order safety, rule 19). */, dailyChest: true /* v2.3.2820: the daily reward is a chest in the bag, opened from the bag (dailychest.js).  The client gates its Open button and its chest_open send on this: against an OLD worker the flag is absent, the day still pays plain gold and no chest can exist; against a NEW worker an old client simply holds a chest it cannot open until Pages redeploys it (the chest waits, it is never lost).  Kill switch: `dailyChest: false` in liveflags un-advertises this AND pays the day in gold again (cadence.js). */, smelting: true /* v2.3.2822: ore smelts into bars at the blacksmith (smelting.js).  The client gates its Smelting rows and its smelt_bar send on this: against an OLD worker the flag is absent and the rows never appear -- that worker has no case for smelt_bar, so it would fall to the default branch and REBROADCAST the request while settling nothing.  Against a NEW worker an old client simply never shows the rows (deploy-order safety, rule 19).  Kill switch: `smelting: false` in liveflags un-advertises this AND refuses every smelt (smelting.js _smeltOff). */, whirlWindup: true /* v2.3.2824: whirlwind arms on the press and strikes 2s later from wherever you are then (abilities.js windupMs); every client in the zone gets ability_windup and draws the ring.  The client gates its RING and its delayed local effect on this: against an OLD worker the flag is absent and the whirlwind fires on the press exactly as before (that worker strikes instantly).  Against a NEW worker an old client plays its effect on the press while the hit lands 2s later -- a visual mismatch until Pages redeploys, never a lost or doubled cast (deploy-order safety, rule 19).  Kill switch: `whirlWindup: false` in liveflags un-advertises this AND makes the worker strike instantly again (_abilWindupOff). */, zoneDepth: true /* v2.3.2790: the monster AI measures in the zone's depth (depth.js) -- on Wind Dunes a monster notices, walks, reaches and is shoved by the curve its body is drawn at.  The client gates its DRAWING of that curve on this, never a send: against an OLD worker the flag is absent and the dunes draw flat (the curve stays behind the ?depth=1 preview), because a worker that still hits from flat distances would make a small far monster strike from outside its own body.  Against a NEW worker an old client simply draws everything full size over a world whose monsters are measuring small -- they reach a little shorter than they look, never further (deploy-order safety, rule 19).  Kill switch: `zoneDepth: false` in liveflags un-advertises this AND turns the server scaling off (depth.js _depthK). */, bowvolley: true /* v2.3.2848: the bow special is THREE white-hot arrows from one shot (playerActions.js), each carrying `part: 3` on its monster_damage so the worker gives each a share of its own capped roll (combat.js; v2.3.2849: BOW_VOLLEY_WORTH / 3 -- two-thirds an arrow, and the client burns 2.5 s instead of 4) -- and there is no send-off blast: arrow_blast is refused as 'retired' while this is live (arrowblast.js _bowVolleyLive).  The client gates the whole volley on this flag, never a part of it: against an OLD worker the flag is absent and the special stays the one arrow with its burn and its blast, because that worker would roll every arrow at full strength.  Against a NEW worker an old client fires its one full-strength arrow (no `part`, so the worker rolls it whole, exactly as before) and its blast is refused -- it keeps the arrow and the burn and loses only the send-off (deploy-order safety, rule 19).  Kill switch: `bowvolley: false` in liveflags (POST /api/admin/flags -- the name is lower case so that route accepts it) un-advertises this AND lets the blast through again, so a client that joins after it gets the old special back whole. */, bigorb: true /* v2.3.2842: the staff special as ONE big bolt (playerActions.js) whose monster_damage carries `orbs: 3` -- the worker rolls three special hits and sums them into one monster_hit (combat.js).  v2.3.2849: ONE roll now, from the bolt's own wide band (STAFF_BOLT.BAND, 0.3-2.5) x the three orbs, and it EXPLODES -- every other monster within 90 px takes a third of it.  Lower case since v2.3.2849 (was bigOrb, never shipped) so it is a kill switch the admin flags route can throw (TRAPS §117): `bigorb: false` in liveflags un-advertises it, and a client that joins after that fires the old three orbs, each rolled and capped as before, with no blast.  A tab that joined earlier keeps its big bolt until it rejoins -- the worker honours `orbs` whatever the flag says, as it honours `part`.  The client gates the big bolt on this: against an OLD worker the flag is absent and the special stays the three-orb volley, because that worker ignores `orbs` and would roll one bolt as one orb, a third of the special.  Against a NEW worker an old client keeps firing three orbs with no `orbs` field, each rolled exactly as before (deploy-order safety, rule 19). */, ..._liveFlags },
