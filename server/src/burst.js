...
export const burstMethods = {
  ...
  _handleElementBurst(session) {
    ...
    for (const m of monsters) {
      ...
      const rolled = this._computeAttackDamage(ps, slot, false, { targetLevel: m.level });  /* v2.3.2659: the edge */
      ...
      applyElementStatus(m, element, session.id, elemAttackStat(ps, 'power', this._prog3CatFor(slot === 'ranged' ? 'bow' : slot), m.level), now, this._attuneMult(ps)); // v2.3.2199: prog3 snapshots `elem`; v2.3.2659: + edge
      targets.push(m.id);
